...
# bash <(curl -qfsSL "https://pub.ajam.dev/repos/Azathothas/Toolpacks/.github/scripts/arm64_v8a_Android/gen_meta.sh")
# bash <(curl -qfsSL "https://raw.githubusercontent.com/Azathothas/Toolpacks/refs/heads/main/.github/scripts/arm64_v8a_Android/gen_meta.sh")
             ...
             if [ -f "$SYSTMP/BUILD.log" ] && [ $(stat -c%s "$SYSTMP/BUILD.log") -gt 10240 ]; then
               ...
               if [ -n "${LOG_BEGIN}" ] && [ -n "${LOG_END}" ]; then
                 ...
                 if [ -f "${SYSTMP}/BIN_LOGS/${C_NAME}.log.txt" ] && [ $(stat -c%s "${SYSTMP}/BIN_LOGS/${C_NAME}.log.txt") -gt 10 ]; then
                   ...
                   jq --arg BIN "$BIN" --arg BUILD_LOG "https://bin.ajam.dev/arm64_v8a_Android/BUILD.log.txt" '.[] |= if .name == $BIN then . + {build_log: $BUILD_LOG} else . end' "$TMPDIR/METADATA.json" > "$TMPDIR/METADATA.tmp" && mv "$TMPDIR/METADATA.tmp" "$TMPDIR/METADATA.json"
                 fi
               fi
             else
               jq --arg BIN "$BIN" --arg BUILD_LOG "https://bin.ajam.dev/arm64_v8a_Android/BUILD.log.txt" '.[] |= if .name == $BIN then . + {build_log: $BUILD_LOG} else . end' "$TMPDIR/METADATA.json" > "$TMPDIR/METADATA.tmp" && mv "$TMPDIR/METADATA.tmp" "$TMPDIR/METADATA.json"
...
if jq --exit-status . "$TMPDIR/METADATA.json.bak" >/dev/null 2>&1; then
   ...
   if [ "$(jq '. | length' "$TMPDIR/METADATA.json")" -gt 10 ]; then
    ...
    #Sync GH Meta 
     ...
     rclone copyto --checksum "${SYSTMP}/GH_TMP/bin.default.png" "r2:/bin/arm64_v8a_Android/bin.default.png" --check-first --checkers 2000 --transfers 1000 --retries="10" --user-agent="$USER_AGENT"
     rclone copyto --checksum "${SYSTMP}/GH_TMP/base.default.png" "r2:/bin/arm64_v8a_Android/Baseutils/base.default.png" --check-first --checkers 2000 --transfers 1000 --retries="10" --user-agent="$USER_AGENT"
    #Sync TLDR
    ...
    #Cleanup
      rclone delete "r2:/bin/arm64_v8a_Android" --min-size 0 --max-size 0
   else
     echo -e "\n[-] FATAL: ($(realpath "$TMPDIR/METADATA.json")) is small (<10)\n"
   fi
else
