...
#Get URlS
curl -qfsSL "https://pub.ajam.dev/repos/Azathothas/Toolpacks/.github/scripts/arm64_v8a_Android/bins/metadata.json" | jq -r '.[].source_url' | sed 's/\.sh$/\.yaml/' | grep -i '\.yaml$' | sort -u -o "$SYSTMP/BUILDURLS"
#Get METADATA.json (bin.ajam.dev/arm64_v8a_Android)
curl -qfsSL "https://bin.ajam.dev/arm64_v8a_Android/METADATA.json.tmp" -o "$SYSTMP/RCLONE/METADATA.json" || curl -qfsSL "https://bin.ajam.dev/arm64_v8a_Android/METADATA.json" -o "$SYSTMP/RCLONE/METADATA.json"
#Get BLAKE3SUM.txt
curl -qfsSL "https://bin.ajam.dev/arm64_v8a_Android/BLAKE3SUM.txt" -o "$SYSTMP/RCLONE/BLAKE3SUM.txt"
#Get SHA256SUM.txt
curl -qfsSL "https://bin.ajam.dev/arm64_v8a_Android/SHA256SUM.txt" -o "$SYSTMP/RCLONE/SHA256SUM.txt"
##Sanity
...
echo -e "\n\n [+] Started Metadata Update at :: $(TZ='Asia/Kathmandu' date +'%A, %Y-%m-%d (%I:%M:%S %p)')\n\n"
 for BUILD_URL in $(cat "$SYSTMP/BUILDURLS" | sed 's/"//g'); do
   echo -e "\n[+] Fetching : $BUILD_URL"
    if curl -qfsSL "$BUILD_URL" -o "$BUILDYAML"; then
      ...
      #Merge with json
       for BIN in $(cat "$SYSTMP/RCLONE/BINS.txt" | sed 's/"//g'); do
         ...
         #BSUM
          B3SUM="$(cat "$SYSTMP/RCLONE/BLAKE3SUM.txt" | grep -i "\.\/$BIN$" | awk '{print $1}' | sort  -u | head -n 1 | sed 's/"//g' | sed 's/^[ \t]*//;s/[ \t]*$//' | sed 's/["'\'']//g' | sed 's/`//g' | tr -d '[:space:]')" && export B3SUM="$B3SUM"
         ...
         #SHA256SUM
          SHA256="$(cat "$SYSTMP/RCLONE/SHA256SUM.txt" | grep -i "\.\/$BIN$" | awk '{print $1}' | sort  -u | head -n 1 | sed 's/"//g' | sed 's/^[ \t]*//;s/[ \t]*$//' | sed 's/["'\'']//g' | sed 's/`//g' | tr -d '[:space:]')" && export SHA256="$SHA256"
         ...
         #Git Meta
          ...
          #Sort & Map
            ...
            jq 'map({name: (.name // ""), description: (.description // ""), download_url: (.download_url // ""), size: (.size // ""), b3sum: (.b3sum // ""), sha256: (.sha256 // ""), build_date: (.build_date // ""), repo_url: (.repo_url // ""), repo_author: (.repo_author // ""), repo_info: (.repo_info // ""), repo_updated: (.repo_updated // ""), repo_released: (.repo_released // ""), repo_version: (.repo_version // ""), repo_stars: (.repo_stars // ""), repo_language: (.repo_language // ""), repo_license: (.repo_license // ""), repo_topics: (.repo_topics // ""), web_url: (.web_url // ""), extra_bins: (.extra_bins // "")})' "$SYSTMP/RCLONE/METADATA.json" > "$SYSTMP/RCLONE/METADATA.tmp" && mv "$SYSTMP/RCLONE/METADATA.tmp" "$SYSTMP/RCLONE/METADATA.json"
          else
          #Sort & Map
            jq 'map({name: (.name // ""), description: (.description // ""), download_url: (.download_url // ""), size: (.size // ""), b3sum: (.b3sum // ""), sha256: (.sha256 // ""), build_date: (.build_date // ""), repo_url: (.repo_url // ""), web_url: (.web_url // ""), extra_bins: (.extra_bins // "")})' "$SYSTMP/RCLONE/METADATA.json" > "$SYSTMP/RCLONE/METADATA.tmp" && mv "$SYSTMP/RCLONE/METADATA.tmp" "$SYSTMP/RCLONE/METADATA.json"
          fi
         #Print json
...
rclone copyto --checksum "$SYSTMP/RCLONE/METADATA.json" "r2:/bin/arm64_v8a_Android/METADATA.json" --check-first --checkers 2000 --transfers 1000 --user-agent="$USER_AGENT"
