   ...
   #curl : A command line tool for transferring files with URL syntax 
     ...
     echo -e "\n\n [+] (Building | Fetching) $BIN :: $SOURCE_URL\n"
      ...
      #Copy 
       ...
       cp "./curl" "$BINDIR/curl" ; cp "./curl" "$BASEUTILSDIR/curl"
       #get cert
       eget "https://curl.se/ca/cacert.pem" --to "$BINDIR/curl_cacert.pem"
      #Test [PREFIX=/data/data/com.termux/files/usr]
       timeout -k 10s 20s docker run --privileged -it --rm --platform="linux/arm64" --network="bridge" -v "$BINDIR:/mnt" "termux/termux-docker:aarch64" "/mnt/curl" --version
      #Cleanup Container
