...
#Strip || Cleanup [$BINDIR]
 ...
 #Strip
 find "$BINDIR" -maxdepth 1 -type f -exec aarch64-linux-gnu-strip --strip-debug --strip-dwo --strip-unneeded --preserve-dates "{}" \; 2>/dev/null
...
#Strip || Cleanup [$BASEUTILSDIR]
 ...
 #Strip
 find "$BASEUTILSDIR" -maxdepth 1 -type f -exec aarch64-linux-gnu-strip --strip-debug --strip-dwo --strip-unneeded --preserve-dates "{}" \; 2>/dev/null
 #Rename anything with *_arm64_v8a*
       ...
       if command -v 7z &> /dev/null && [ -d "$BINDIR" ] && [ "$(find "$BINDIR" -mindepth 1 -print -quit 2>/dev/null)" ]; then
          ...
          ##Fetch&Sync [$BINDIR]
            ...
            #Strip || Cleanup
             ...
             find "$BINDIR" -type f -name '*_Android' -exec sh -c 'newname=$(echo "$1" | sed "s/_arm64_v8a_Android//"); mv "$1" "$newname"' sh {} \;
            #File
              cd "$BINDIR" && find "./" -maxdepth 1 -type f | grep -v -E '\.jq$|\.txt$|\.upx$' | sort | xargs file > "$SYSTMP/arm64_v8a_Android_FILE"
            ...
            #BLAKE3SUM
              cd "$BINDIR" && find "./" -maxdepth 1 -type f | grep -v -E '\.jq$|\.txt$|\.upx$' | sort | xargs b3sum > "$SYSTMP/arm64_v8a_Android_BLAKE3SUM"
            ...
            #SHA256SUM
              cd "$BINDIR" && find "./" -maxdepth 1 -type f | grep -v -E '\.jq$|\.txt$|\.upx$' | sort | xargs sha256sum > "$SYSTMP/arm64_v8a_Android_SHA256SUM"
          ...
          ##Fetch&Sync [$BASEUTILSDIR]
            ...
            #Strip || Cleanup
             ...
             #Strip
             find "$BASEUTILSDIR" -type f -exec aarch64-linux-gnu-strip --strip-debug --strip-dwo --strip-unneeded --preserve-dates "{}" \; 2>/dev/null
             #Rename anything with *_arm64_v8a*
             find "$BASEUTILSDIR" -type f -name '*_Android' -exec sh -c 'newname=$(echo "$1" | sed "s/_arm64_v8a_Android//"); mv "$1" "$newname"' sh {} \;
            #File
              cd "$BASEUTILSDIR" && find "./" -type f | grep -v '.txt' | sort | xargs file > "$SYSTMP/arm64_v8a_Android_Baseutils_FILE"
            ...
            #Size (Dust)
              dust --only-file --no-percent-bars --no-colors --ignore_hidden --reverse --number-of-lines 99999999 "$BASEUTILSDIR" | tee "$SYSTMP/arm64_v8a_Android_Baseutils_SIZE.txt"
            ...
            #BLAKE3SUM
              cd "$BASEUTILSDIR" && find "./" -type f | grep -v '.txt' | sort | xargs b3sum > "$SYSTMP/arm64_v8a_Android_Baseutils_BLAKE3SUM"
            ...
            #SHA256SUM
              cd "$BASEUTILSDIR" && find "./" -type f | grep -v '.txt' | sort | xargs sha256sum > "$SYSTMP/arm64_v8a_Android_Baseutils_SHA256SUM"
...
if [ "${UPX_PACK}" = "YES" ]; then
   ...
   #Pack:: https://github.com/upx/upx/blob/devel/doc/upx-doc.txt
     if command -v upx &> /dev/null && [ -d "${UPX_PACK_DIR}" ] && [ "$(find "${UPX_PACK_DIR}" -mindepth 1 -print -quit 2>/dev/null)" ]; then
       ...
       find . -type f ! -name '*.upx' -exec file -i '{}' \; | grep "application/.*executable" | cut -d':' -f1 | xargs realpath | sort -u -o "$SYSTMP/upx_input.txt"
