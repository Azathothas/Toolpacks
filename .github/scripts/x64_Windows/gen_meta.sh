...
#Get URlS
curl -qfsSL "https://pub.ajam.dev/repos/Azathothas/Toolpacks/.github/scripts/x64_Windows/bins/metadata.json" | jq -r '.[].source_url' | sed 's/\.ps1$/\.yaml/' | sort -u -o "$SYSTMP/BUILDURLS"
#Get METADATA.json (bin.ajam.dev/x64_Windows)
curl -qfsSL "https://bin.ajam.dev/x64_Windows/METADATA.json.tmp" -o "$SYSTMP/RCLONE/METADATA.json" || curl -qfsSL "https://bin.ajam.dev/x64_Windows/METADATA.json" -o "$SYSTMP/RCLONE/METADATA.json"
#Get BLAKE3SUM.txt
curl -qfsSL "https://bin.ajam.dev/x64_Windows/BLAKE3SUM.txt" -o "$SYSTMP/RCLONE/BLAKE3SUM.txt"
#Get SHA256SUM.txt
curl -qfsSL "https://bin.ajam.dev/x64_Windows/SHA256SUM.txt" -o "$SYSTMP/RCLONE/SHA256SUM.txt"
##Sanity
...
rclone copyto --checksum "$SYSTMP/RCLONE/METADATA.json" "r2:/bin/x64_Windows/METADATA.json" --check-first --checkers 2000 --transfers 1000 --user-agent="$USER_AGENT"
