...
# bash <(curl -qfsSL "https://pub.ajam.dev/repos/Azathothas/Toolpacks/.github/scripts/x64_Windows/gen_meta.sh")
# bash <(curl -qfsSL "https://raw.githubusercontent.com/Azathothas/Toolpacks/refs/heads/main/.github/scripts/x64_Windows/gen_meta.sh")
   ...
   echo -e "\n[+] Fetching : $BUILD_URL"
    if curl -qfsSL "$BUILD_URL" -o "$BUILDYAML" &> /dev/null; then
      ...
      if [ "$(yq e '.path' "$BUILDYAML")" = "/" ]; then
         ...
         #export Build Script
          BUILD_SCRIPT="$(echo "$BUILD_URL" | sed 's|https://pub.ajam.dev/repos|https://github.com|; s|/Toolpacks|/Toolpacks/tree/main|; s|\.yaml$|.ps1|')" && export BUILD_SCRIPT="$BUILD_SCRIPT"
         #export Notes (Note)
             ...
             if [ -f "$SYSTMP/BUILD.log" ] && [ $(stat -c%s "$SYSTMP/BUILD.log") -gt 10240 ]; then
               ...
               if [ -n "${LOG_BEGIN}" ] && [ -n "${LOG_END}" ]; then
                 ...
                 if [ -f "${SYSTMP}/BIN_LOGS/${C_NAME}.log.txt" ] && [ $(stat -c%s "${SYSTMP}/BIN_LOGS/${C_NAME}.log.txt") -gt 10 ]; then
                   ...
                   jq --arg BIN "$BIN" --arg BUILD_LOG "https://bin.ajam.dev/x64_Windows/BUILD.log.txt" '.[] |= if .name == $BIN then . + {build_log: $BUILD_LOG} else . end' "$TMPDIR/METADATA.json" > "$TMPDIR/METADATA.tmp" && mv "$TMPDIR/METADATA.tmp" "$TMPDIR/METADATA.json"
                 fi
               fi
             else
                 jq --arg BIN "$BIN" --arg BUILD_LOG "https://bin.ajam.dev/x64_Windows/BUILD.log.txt" '.[] |= if .name == $BIN then . + {build_log: $BUILD_LOG} else . end' "$TMPDIR/METADATA.json" > "$TMPDIR/METADATA.tmp" && mv "$TMPDIR/METADATA.tmp" "$TMPDIR/METADATA.json"
...
if jq --exit-status . "$TMPDIR/METADATA.json.bak" >/dev/null 2>&1; then
   ...
   if [ "$(jq '. | length' "$TMPDIR/METADATA.json")" -gt 100 ]; then
    ...
    #Cleanup
      rclone delete "r2:/bin/x64_Windows" --min-size 0 --max-size 0
   else
