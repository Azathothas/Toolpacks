...
#Get URlS
curl -qfsSL "https://pub.ajam.dev/repos/Azathothas/Toolpacks/.github/scripts/x64_Windows/bins/metadata.json" | jq -r '.[].source_url' | sed 's/\.ps1$/\.yaml/' | grep -i '\.yaml$' | sort -u -o "$TMPDIR/BUILDURLS"
#Get METADATA.json (bin.ajam.dev/x64_Windows)
curl -qfsSL "https://bin.ajam.dev/x64_Windows/METADATA.json.tmp" -o "$TMPDIR/METADATA.json" || curl -qfsSL "https://bin.ajam.dev/x64_Windows/METADATA.json" -o "$TMPDIR/METADATA.json"
#Get BLAKE3SUM.txt
curl -qfsSL "https://bin.ajam.dev/x64_Windows/BLAKE3SUM.txt" -o "$TMPDIR/BLAKE3SUM.txt"
#Get SHA256SUM.txt
curl -qfsSL "https://bin.ajam.dev/x64_Windows/SHA256SUM.txt" -o "$TMPDIR/SHA256SUM.txt"
##Sanity
