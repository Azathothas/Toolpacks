...
if ($env:SKIP_BUILD -eq "NO") {
   #monolith: ⬛️ CLI tool for saving complete web pages as a single HTML file 
    $env:BIN = "monolith" # Name of final binary/pkg/cli, sometimes differs from $REPO
    $env:SOURCE_URL = "https://github.com/Y2Z/monolith" # github/gitlab/homepage/etc for $BIN
    Write-Output "`n`n [+] (Building | Fetching) $env:BIN :: $env:SOURCE_URL`n"
    #Build
      Push-Location (& $TMPDIRS) ; git clone --quiet --filter "blob:none" "https://github.com/Y2Z/monolith" ; Set-Location "./monolith" ; (Resolve-Path ".\").Path
      ...
      Copy-Item "./target/$env:RUST_TARGET/release/monolith.exe" "$env:BINDIR/monolith.exe"
      file.exe "./target/$env:RUST_TARGET/release/monolith.exe" ; (Get-Item -Path "./target/$env:RUST_TARGET/release/monolith.exe").Length | ForEach-Object { "{0:N2} MB" -f ($_ / 1MB) }
      #objdump.exe -x "./target/$env:RUST_TARGET/release/monolith.exe" | Select-String "DLL Name:" 
      wldd.exe "./target/$env:RUST_TARGET/release/monolith.exe" | Sort-Object -Unique
