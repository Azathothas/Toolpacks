...
if ($env:SKIP_BUILD -eq "NO") {
   #shell2http : Executing shell commands via HTTP server
    $env:BIN = "shell2http" # Name of final binary/pkg/cli, sometimes differs from $REPO
    $env:SOURCE_URL = "https://github.com/msoap/shell2http" # github/gitlab/homepage/etc for $BIN
    Write-Output "`n`n [+] (Building | Fetching) $env:BIN :: $env:SOURCE_URL`n"
    #Fetch 
     eget "$env:SOURCE_URL" --asset "windows" --asset "amd64" --asset "zip" --to "$env:BINDIR/shell2http.exe"
}
#-------------------------------------------------------#
