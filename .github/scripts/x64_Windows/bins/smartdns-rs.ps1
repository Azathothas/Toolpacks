...
if ($env:SKIP_BUILD -eq "NO") {
   #smartdns-rs : local DNS server (Dnsmasq like) written in rust to obtain the fastest website IP for the best Internet experience, supports DoT, DoQ, DoH, DoH3. 
    $env:BIN = "smartdns-rs" # Name of final binary/pkg/cli, sometimes differs from $REPO
    $env:SOURCE_URL = "https://github.com/mokeyish/smartdns-rs" # github/gitlab/homepage/etc for $BIN
    Write-Output "`n`n [+] (Building | Fetching) $env:BIN :: $env:SOURCE_URL`n"
    #Fetch 
     eget "$env:SOURCE_URL" --asset "windows" --asset "x86_64" --asset "msvc" --asset "zip" --asset "^txt" --file "smartdns.exe" --to "$env:BINDIR/smartdns-rs.exe"
}
#-------------------------------------------------------#
