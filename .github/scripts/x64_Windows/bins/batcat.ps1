...
if ($env:SKIP_BUILD -eq "NO") {
   #batcat: cat with colors & syntax highlights
    $env:BIN = "bat" # Name of final binary/pkg/cli, sometimes differs from $REPO
    $env:SOURCE_URL = "https://github.com/sharkdp/bat" # github/gitlab/homepage/etc for $BIN
    Write-Output "`n`n [+] (Building | Fetching) $env:BIN :: $env:SOURCE_URL`n"
    #Fetch 
     eget "$env:SOURCE_URL" --asset "win" --asset "64" --asset "msvc" --asset "zip" --to "$env:BINDIR/bat.exe"
     eget "$env:SOURCE_URL" --asset "win" --asset "64" --asset "msvc" --asset "zip" --to "$env:BINDIR/batcat.exe"
}
#-------------------------------------------------------#
