...
if ($env:SKIP_BUILD -eq "NO") {
   #dufs : A file server that supports static serving, uploading, searching, accessing control, webdav
    $env:BIN = "dufs" # Name of final binary/pkg/cli, sometimes differs from $REPO
    $env:SOURCE_URL = "https://github.com/sigoden/dufs" # github/gitlab/homepage/etc for $BIN
    Write-Output "`n`n [+] (Building | Fetching) $env:BIN :: $env:SOURCE_URL`n"
    #Fetch 
     eget "$env:SOURCE_URL" --asset "windows" --asset "x86_64" --asset "msvc" --asset "zip" --to "$env:BINDIR/dufs.exe"
}
#-------------------------------------------------------#
