...
if ($env:SKIP_BUILD -eq "NO") {
   #iperf3 : Command-line JSON processor 
    $env:BIN = "iperf3" # Name of final binary/pkg/cli, sometimes differs from $REPO
    $env:SOURCE_URL = "https://github.com/userdocs/iperf3-static" # github/gitlab/homepage/etc for $BIN
    Write-Output "`n`n [+] (Building | Fetching) $env:BIN :: $env:SOURCE_URL`n"
    #Fetch 
     eget "$env:SOURCE_URL" --asset "windows" --asset "amd64" --asset "exe" --to "$env:BINDIR/iperf3.exe"
}
#-------------------------------------------------------#
...
if ($env:SKIP_BUILD -eq "NO") {
   #iperf3 : A tool for network performance measurement and tuning 
    ...
    Write-Output "`n`n [+] (Building | Fetching) $env:BIN :: $env:SOURCE_URL`n"
    #Fetch 
     #This requires cygwin DLL: https://www.cygwin.com/install.html 
     eget "$env:SOURCE_URL" --asset "win" --asset "amd64" --asset "openssl" --asset "zip" --to "$env:BINDIR/iperf3.exe"
}
#-------------------------------------------------------#
