...
if ($env:SKIP_BUILD -eq "NO") {
   #ripgrepcat: rust-grep
    $env:BIN = "ripgrep" # Name of final binary/pkg/cli, sometimes differs from $REPO
    $env:SOURCE_URL = "https://github.com/BurntSushi/ripgrep" # github/gitlab/homepage/etc for $BIN
    Write-Output "`n`n [+] (Building | Fetching) $env:BIN :: $env:SOURCE_URL`n"
    #Fetch 
     $EGET_OUT = & { eget "$env:SOURCE_URL" --asset "win" --asset "64" --asset "msvc" --asset "zip" --asset "^sha" --to "$env:TEMP" } 2>&1 ; $env:EGET_OUT = "$EGET_OUT"
     ...
     eget "$env:EGET_LINK" --file "rg.exe" --to "$env:BINDIR/rg.exe"
     eget "$env:SOURCE_URL" --file "rg.exe" --to "$env:BINDIR/ripgrep.exe"
}
#-------------------------------------------------------#
