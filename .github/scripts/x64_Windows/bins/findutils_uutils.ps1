...
if ($env:SKIP_BUILD -eq "NO") {
   #findutils : Rust implementation of findutils 
    $env:BIN = "findutils" # Name of final binary/pkg/cli, sometimes differs from $REPO
    $env:SOURCE_URL = "https://github.com/uutils/findutils" # github/gitlab/homepage/etc for $BIN
    Write-Output "`n`n [+] (Building | Fetching) $env:BIN :: $env:SOURCE_URL`n"
    #Build
      Push-Location (& $TMPDIRS) ; git clone --quiet --filter "blob:none" "https://github.com/uutils/findutils" ; Set-Location "./findutils" ; (Resolve-Path ".\").Path
      ...
      Copy-Item "./target/$env:RUST_TARGET/release/find.exe" "$env:BINDIR/find.exe"
      Copy-Item "./target/$env:RUST_TARGET/release/find.exe" "$env:BINDIR/find-rs.exe"
