...
if ($env:SKIP_BUILD -eq "NO") {
   #cfspeedtest : Unofficial CLI for speed.cloudflare.com
    $env:BIN = "cfspeedtest" # Name of final binary/pkg/cli, sometimes differs from $REPO
    $env:SOURCE_URL = "https://github.com/code-inflation/cfspeedtest" # github/gitlab/homepage/etc for $BIN
    Write-Output "`n`n [+] (Building | Fetching) $env:BIN :: $env:SOURCE_URL`n"
    #Fetch 
     eget "$env:SOURCE_URL" --asset "windows" --asset "x64" --asset "msvc" --asset "zip" --to "$env:BINDIR/cfspeedtest.exe"
}
#-------------------------------------------------------#
