...
if ($env:SKIP_BUILD -eq "NO") {
   #ooniprobe : OONI Probe Engine and CLI
    $env:BIN = "ooniprobe"
    $env:SOURCE_URL = "https://github.com/ooni/probe-cli"
    Write-Output "`n`n [+] (Building | Fetching) $env:BIN :: $env:SOURCE_URL`n"
    #Fetch 
     eget "$env:SOURCE_URL" --asset "windows" --asset "amd64" --asset "miniooni" --to "$env:BINDIR/miniooni.exe"
     eget "$env:SOURCE_URL" --asset "windows" --asset "amd64" --asset "ooniprobe" --to "$env:BINDIR/ooniprobe.exe"
}
#-------------------------------------------------------#
