...
if ($env:SKIP_BUILD -eq "NO") {
   #pgrok : multi-tenant HTTP/TCP reverse tunnel solution through SSH remote port forwarding
    $env:BIN = "pgrok" # Name of final binary/pkg/cli, sometimes differs from $REPO
    $env:SOURCE_URL = "https://github.com/pgrok/pgrok" # github/gitlab/homepage/etc for $BIN
    Write-Output "`n`n [+] (Building | Fetching) $env:BIN :: $env:SOURCE_URL`n"
    #Fetch 
     eget "$env:SOURCE_URL" --asset "windows" --asset "amd64" --asset "zip" --asset "^pgrokd" --file "pgrok.exe" --to "$env:BINDIR/pgrok.exe"
     eget "$env:SOURCE_URL" --asset "windows" --asset "amd64" --asset "zip" --asset "pgrokd" --file "pgrokd.exe" --to "$env:BINDIR/pgrokd.exe"
}
#-------------------------------------------------------#
