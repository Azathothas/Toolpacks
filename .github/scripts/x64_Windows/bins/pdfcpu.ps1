...
if ($env:SKIP_BUILD -eq "NO") {
   #pdfcpu : PDF processor
    $env:BIN = "pdfcpu" # Name of final binary/pkg/cli, sometimes differs from $REPO
    $env:SOURCE_URL = "https://github.com/pdfcpu/pdfcpu" # github/gitlab/homepage/etc for $BIN
    Write-Output "`n`n [+] (Building | Fetching) $env:BIN :: $env:SOURCE_URL`n"
    #Fetch 
     eget "$env:SOURCE_URL" --asset "Windows" --asset "x86_64" --asset "zip" --to "$env:BINDIR/pdfcpu.exe"
}
#-------------------------------------------------------#
