...
if ($env:SKIP_BUILD -eq "NO") {
   #fastfetch : Like neofetch, but much faster because written in C.
    $env:BIN = "fastfetch" # Name of final binary/pkg/cli, sometimes differs from $REPO
    $env:SOURCE_URL = "https://github.com/fastfetch-cli/fastfetch" # github/gitlab/homepage/etc for $BIN
    Write-Output "`n`n [+] (Building | Fetching) $env:BIN :: $env:SOURCE_URL`n"
    #Fetch 
     eget "$env:SOURCE_URL" --asset "windows" --asset "amd64" --asset "zip" --file "fastfetch.exe" --to "$env:BINDIR/fastfetch.exe"
     eget "$env:SOURCE_URL" --asset "windows" --asset "amd64" --asset "zip" --file "flashfetch.exe" --to "$env:BINDIR/flashfetch.exe"
}
#-------------------------------------------------------#
