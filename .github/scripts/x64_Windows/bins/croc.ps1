...
if ($env:SKIP_BUILD -eq "NO") {
   #croc : Easily and securely send things from one computer to another 🐊 📦
    $env:BIN = "croc" # Name of final binary/pkg/cli, sometimes differs from $REPO
    $env:SOURCE_URL = "https://github.com/schollz/croc" # github/gitlab/homepage/etc for $BIN
    Write-Output "`n`n [+] (Building | Fetching) $env:BIN :: $env:SOURCE_URL`n"
    #Fetch 
     eget "$env:SOURCE_URL" --asset "Windows" --asset "64" --asset "zip" --asset "^ARM" --asset "^Windows7" --to "$env:BINDIR/croc.exe"
}
#-------------------------------------------------------#
