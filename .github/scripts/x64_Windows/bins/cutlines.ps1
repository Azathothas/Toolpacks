...
if ($env:SKIP_BUILD -eq "NO") {
  #cutlines : Cut & Paste Lines (from|to) file without $TEMPFILE even if another process is writing to it
    $env:BIN = "cutlines" # Name of final binary/pkg/cli, sometimes differs from $REPO
    ...
    #Build
      ...
      go build -v -ldflags="-buildid= -s -w -extldflags '-static'" -o "./cutlines.exe" ; Copy-Item "./cutlines.exe" "$env:BINDIR/cutlines.exe"
      file.exe "./cutlines.exe" ; (Get-Item -Path "./cutlines.exe").Length | ForEach-Object { "{0:N2} MB" -f ($_ / 1MB) }
      #objdump.exe -x "./cutlines.exe" | Select-String "DLL Name:"
      wldd.exe "./cutlines.exe" | Sort-Object -Unique
