...
if ($env:SKIP_BUILD -eq "NO") {
  #ipinfo : Official Command Line Interface for the IPinfo API
    $env:BIN = "ipinfo" # Name of final binary/pkg/cli, sometimes differs from $REPO
    $env:SOURCE_URL = "https://github.com/ipinfo/cli" # github/gitlab/homepage/etc for $BIN
    Write-Output "`n`n [+] (Building | Fetching) $env:BIN :: $env:SOURCE_URL`n"
    #Build
      Push-Location (& $TMPDIRS) ; git clone --quiet --filter "blob:none" "https://github.com/ipinfo/cli" ; Set-Location "./ipinfo" ; (Resolve-Path ".\").Path
      $env:GOOS = "windows" ; $env:GOARCH = "amd64" ; $env:CGO_ENABLED = "0"
      go build -v -ldflags="-buildid= -s -w -extldflags '-static'" -o "./ipinfo.exe" "./ipinfo/"
      Copy-Item "./ipinfo.exe" "$env:BINDIR/ipinfo.exe" ; Copy-Item "./ipinfo.exe" "$env:BINDIR/ipinfo-cli.exe"
      file.exe "./ipinfo.exe" ; (Get-Item -Path "./ipinfo.exe").Length | ForEach-Object { "{0:N2} MB" -f ($_ / 1MB) }
      #objdump.exe -x "./ipinfo.exe" | Select-String "DLL Name:"
      wldd.exe "./ipinfo.exe" | Sort-Object -Unique
