...
if ($env:SKIP_BUILD -eq "NO") {
   #pandoc :  Universal markup converter 
    $env:BIN = "pandoc" # Name of final binary/pkg/cli, sometimes differs from $REPO
    $env:SOURCE_URL = "https://github.com/jgm/pandoc" # github/gitlab/homepage/etc for $BIN
    Write-Output "`n`n [+] (Building | Fetching) $env:BIN :: $env:SOURCE_URL`n"
    #Fetch 
     eget "$env:SOURCE_URL" --asset "windows" --asset "x86_64" --asset "zip" --to "$env:BINDIR/pandoc.exe"
}
#-------------------------------------------------------#
