...
if ($env:SKIP_BUILD -eq "NO") {
   #dbbench : a simple database benchmarking tool which supports several databases and own scripts
    $env:BIN = "dbbench" # Name of final binary/pkg/cli, sometimes differs from $REPO
    $env:SOURCE_URL = "https://github.com/sj14/dbbench" # github/gitlab/homepage/etc for $BIN
    Write-Output "`n`n [+] (Building | Fetching) $env:BIN :: $env:SOURCE_URL`n"
    #Fetch 
     eget "$env:SOURCE_URL" --asset "windows" --asset "amd64" --asset "exe" --to "$env:BINDIR/dbbench.exe"
}
#-------------------------------------------------------#
