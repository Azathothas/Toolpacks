...
if ($env:SKIP_BUILD -eq "NO") {
   #alist : A file list/WebDAV program that supports multiple storages 
    $env:BIN = "alist" # Name of final binary/pkg/cli, sometimes differs from $REPO
    $env:SOURCE_URL = "https://github.com/alist-org/alist" # github/gitlab/homepage/etc for $BIN
    Write-Output "`n`n [+] (Building | Fetching) $env:BIN :: $env:SOURCE_URL`n"
    #Fetch 
     eget "$env:SOURCE_URL" --asset "windows" --asset "amd64" --asset "zip" --asset "^upx" --to "$env:BINDIR/alist.exe"
}
#-------------------------------------------------------#
