...
if ($env:SKIP_BUILD -eq "NO") {
   #cheat : create and view interactive cheatsheets on the command-line
    $env:BIN = "cheat" # Name of final binary/pkg/cli, sometimes differs from $REPO
    $env:SOURCE_URL = "https://github.com/cheat/cheat" # github/gitlab/homepage/etc for $BIN
    Write-Output "`n`n [+] (Building | Fetching) $env:BIN :: $env:SOURCE_URL`n"
    #Fetch 
     eget "$env:SOURCE_URL" --asset "windows" --asset "amd64" --asset "zip" --to "$env:BINDIR/cheat.exe"
}
#-------------------------------------------------------#
