...
if ($env:SKIP_BUILD -eq "NO") {
   #micro : A modern and intuitive terminal-based text editor
    $env:BIN = "micro" # Name of final binary/pkg/cli, sometimes differs from $REPO
    $env:SOURCE_URL = "https://github.com/zyedidia/micro" # github/gitlab/homepage/etc for $BIN
    Write-Output "`n`n [+] (Building | Fetching) $env:BIN :: $env:SOURCE_URL`n"
    #Fetch 
     eget "$env:SOURCE_URL" --asset "win" --asset "64" --asset "zip" --to "$env:BINDIR/micro.exe"
}
#-------------------------------------------------------#
