...
if ($env:SKIP_BUILD -eq "NO") {
   #brook : A cross-platform programmable network tool 
    $env:BIN = "brook" # Name of final binary/pkg/cli, sometimes differs from $REPO
    $env:SOURCE_URL = "https://github.com/txthinking/brook" # github/gitlab/homepage/etc for $BIN
    Write-Output "`n`n [+] (Building | Fetching) $env:BIN :: $env:SOURCE_URL`n"
    #Fetch 
     eget "$env:SOURCE_URL" --asset "windows" --asset "amd64" --asset "exe" --to "$env:BINDIR/brook.exe"
}
#-------------------------------------------------------#
