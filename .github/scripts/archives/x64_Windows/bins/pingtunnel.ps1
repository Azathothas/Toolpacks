...
if ($env:SKIP_BUILD -eq "NO") {
   #pingtunnel : Pingtunnel is a tool that send TCP/UDP traffic over ICMP
    $env:BIN = "pingtunnel" # Name of final binary/pkg/cli, sometimes differs from $REPO
    $env:SOURCE_URL = "https://github.com/esrrhs/pingtunnel" # github/gitlab/homepage/etc for $BIN
    Write-Output "`n`n [+] (Building | Fetching) $env:BIN :: $env:SOURCE_URL`n"
    #Fetch 
     eget "$env:SOURCE_URL" --asset "windows" --asset "amd64" --asset "zip" --to "$env:BINDIR/pingtunnel.exe"
}
#-------------------------------------------------------#
