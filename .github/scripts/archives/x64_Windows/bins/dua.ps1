...
if ($env:SKIP_BUILD -eq "NO") {
   #dua-cli : (du) View disk space usage and delete unwanted data, fast.
    $env:BIN = "dua" # Name of final binary/pkg/cli, sometimes differs from $REPO
    $env:SOURCE_URL = "https://github.com/Byron/dua-cli" # github/gitlab/homepage/etc for $BIN
    Write-Output "`n`n [+] (Building | Fetching) $env:BIN :: $env:SOURCE_URL`n"
    #Fetch 
     eget "$env:SOURCE_URL" --asset "windows" --asset "x86_64" --asset "msvc" --asset "zip" --to "$env:BINDIR/dua.exe"
}
#-------------------------------------------------------#
