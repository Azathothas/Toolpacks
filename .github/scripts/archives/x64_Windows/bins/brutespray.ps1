...
if ($env:SKIP_BUILD -eq "NO") {
   #brutespray : Command-line JSON processor 
    $env:BIN = "brutespray" # Name of final binary/pkg/cli, sometimes differs from $REPO
    $env:SOURCE_URL = "https://github.com/x90skysn3k/brutespray" # github/gitlab/homepage/etc for $BIN
    Write-Output "`n`n [+] (Building | Fetching) $env:BIN :: $env:SOURCE_URL`n"
    #Fetch 
     eget "$env:SOURCE_URL" --asset "windows" --asset "amd64" --asset "exe" --to "$env:BINDIR/brutespray.exe"
}
#-------------------------------------------------------#
