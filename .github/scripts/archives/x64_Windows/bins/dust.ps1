...
if ($env:SKIP_BUILD -eq "NO") {
   #dust :  A more intuitive version of du in rust
    $env:BIN = "dust" # Name of final binary/pkg/cli, sometimes differs from $REPO
    $env:SOURCE_URL = "https://github.com/bootandy/dust" # github/gitlab/homepage/etc for $BIN
    Write-Output "`n`n [+] (Building | Fetching) $env:BIN :: $env:SOURCE_URL`n"
    #Fetch 
     eget "$env:SOURCE_URL" --asset "windows" --asset "x86_64" --asset "msvc" --asset "zip" --to "$env:BINDIR/dust.exe"
}
#-------------------------------------------------------#
