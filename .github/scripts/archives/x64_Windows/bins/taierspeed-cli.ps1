...
if ($env:SKIP_BUILD -eq "NO") {
  #taierspeed-cli : Speedtest cli for Tel Speed Test
    $env:BIN = "taierspeed-cli" # Name of final binary/pkg/cli, sometimes differs from $REPO
    $env:SOURCE_URL = "https://github.com/ztelliot/taierspeed-cli" # github/gitlab/homepage/etc for $BIN
    Write-Output "`n`n [+] (Building | Fetching) $env:BIN :: $env:SOURCE_URL`n"
    #Fetch 
     eget "$env:SOURCE_URL" --asset "windows" --asset "amd64" --asset "cli" --to "$env:BINDIR/taierspeed-cli.exe"
}
#-------------------------------------------------------#
