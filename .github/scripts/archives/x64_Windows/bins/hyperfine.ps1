...
if ($env:SKIP_BUILD -eq "NO") {
   #hyperfine: A command-line benchmarking tool 
    $env:BIN = "hyperfine" # Name of final binary/pkg/cli, sometimes differs from $REPO
    $env:SOURCE_URL = "https://github.com/sharkdp/hyperfine" # github/gitlab/homepage/etc for $BIN
    Write-Output "`n`n [+] (Building | Fetching) $env:BIN :: $env:SOURCE_URL`n"
    #Fetch 
     eget "$env:SOURCE_URL" --asset "windows" --asset "x86_64" --asset "msvc" --asset "zip" --to "$env:BINDIR/hyperfine.exe"
}
#-------------------------------------------------------#
