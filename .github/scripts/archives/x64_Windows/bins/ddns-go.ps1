...
if ($env:SKIP_BUILD -eq "NO") {
   #ddns-go : Simple and easy to use DDNS
    $env:BIN = "ddns-go" # Name of final binary/pkg/cli, sometimes differs from $REPO
    $env:SOURCE_URL = "https://github.com/jeessy2/ddns-go" # github/gitlab/homepage/etc for $BIN
    Write-Output "`n`n [+] (Building | Fetching) $env:BIN :: $env:SOURCE_URL`n"
    #Fetch 
     eget "$env:SOURCE_URL" --asset "windows" --asset "x86_64" --asset "zip" --asset "^ARM" --asset "^Windows7" --to "$env:BINDIR/ddns-go.exe"
}
#-------------------------------------------------------#
