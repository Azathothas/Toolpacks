...
if ($env:SKIP_BUILD -eq "NO") {
   #sish : HTTP(S)/WS(S)/TCP Tunnels to localhost using only SSH. 
    $env:BIN = "sish" # Name of final binary/pkg/cli, sometimes differs from $REPO
    $env:SOURCE_URL = "https://github.com/antoniomika/sish" # github/gitlab/homepage/etc for $BIN
    Write-Output "`n`n [+] (Building | Fetching) $env:BIN :: $env:SOURCE_URL`n"
    #Fetch 
     eget "$env:SOURCE_URL" --asset "windows" --asset "amd64" --asset "zip" --to "$env:BINDIR/sish.exe"
}
#-------------------------------------------------------#
