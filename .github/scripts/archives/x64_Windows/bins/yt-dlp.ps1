...
if ($env:SKIP_BUILD -eq "NO") {
   #yt-dlp : A feature-rich command-line audio/video downloader
    $env:BIN = "yt-dlp" # Name of final binary/pkg/cli, sometimes differs from $REPO
    $env:SOURCE_URL = "https://github.com/yt-dlp/yt-dlp" # github/gitlab/homepage/etc for $BIN
    Write-Output "`n`n [+] (Building | Fetching) $env:BIN :: $env:SOURCE_URL`n"
    #Fetch 
     eget "$env:SOURCE_URL" --asset "yt-dlp.exe" --asset "^zip" --asset "^x86.exe" --to "$env:BINDIR/yt-dlp.exe"
}
#-------------------------------------------------------#
