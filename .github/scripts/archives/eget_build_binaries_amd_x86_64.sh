...
if ! command -v eget &> /dev/null; then
   ...
   if [ -f "./eget" ]; then
     ...
     #Use GH api
      ...
      find . -type f -name 'eget*' -exec strip --strip-debug --strip-dwo --strip-unneeded --preserve-dates "{}" \; >/dev/null 2>&1
      find . -type f -name 'eget' -exec mv {} "$HOME/bin/eget" \;
      chmod +xwr "$HOME/bin/eget"
      popd >/dev/null 2>&1
      #Use bin.ajam.dev
      if ! command -v eget &> /dev/null; then
         curl -qfsSL "https://bin.ajam.dev/x86_64_Linux" -o "$HOME/bin/eget" ; chmod +xwr "$HOME/bin/eget"
      fi
   fi
else
