...
#rClone Upload to R2 (bin.ajam.dev/x86_64_Linux) (x86_64_Linux) [Binaries]
 if command -v rclone &> /dev/null && [ -s "$HOME/.config/rclone/rclone.conf" ] && [ -d "$BINDIR" ] && [ "$(find "$BINDIR" -mindepth 1 -print -quit 2>/dev/null)" ]; then
    #Upload [$BINDIR]
      ...
      cd "$BINDIR" && rclone copy "." "r2:/bin/x86_64_Linux/" --user-agent="$USER_AGENT" --buffer-size="10M" --s3-upload-concurrency="50" --s3-chunk-size="10M" --multi-thread-streams="50" --checkers="2000" --transfers="100" --retries="10" --check-first --checksum --copy-links --fast-list --progress && sleep 60
    #Upload [$BASEUTILSDIR]
      echo -e "\n[+] Uploading Results to R2 (Baseutils)\n"
      cd "$BASEUTILSDIR" && rclone copy "." "r2:/bin/x86_64_Linux/Baseutils/" --user-agent="$USER_AGENT" --buffer-size="10M" --s3-upload-concurrency="50" --s3-chunk-size="10M" --multi-thread-streams="50" --checkers="2000" --transfers="100" --retries="10" --check-first --checksum --copy-links --fast-list --progress && sleep 60
    ##Archive Binaries (.7z) (x86_64_Linux) Bins [Downstreamed RCLONE]
       if command -v 7z &> /dev/null && [ -d "$BINDIR" ] && [ "$(find "$BINDIR" -mindepth 1 -print -quit 2>/dev/null)" ]; then
          ...
          ##Fetch&Sync [$BINDIR]
             ...
             rclone copy "r2:/bin/x86_64_Linux/" "." --exclude="Baseutils/**" --exclude="*.7z" --exclude="*.gz" --exclude="*.jq" --exclude="*.json" --exclude="*.log" --exclude="*.md" --exclude="*.tar" --exclude="*.tgz" --exclude="*.tmp" --exclude="*.txt" --exclude="*.upx" --exclude="*.zip" --user-agent="$USER_AGENT" --buffer-size="10M" --s3-upload-concurrency="50" --s3-chunk-size="10M" --multi-thread-streams="50" --checkers="2000" --transfers="100" --retries="10" --check-first --checksum --copy-links --fast-list --progress && sleep 60
             #Strip || Cleanup
              #Chmod +xwr
              find "$BINDIR" -maxdepth 1 -type f -exec chmod +xwr {} \; 2>/dev/null
              #Strip
              ...
              find "$BINDIR" -type f -name '*_Linux' -exec sh -c 'newname=$(echo "$1" | sed "s/_amd_x86_64_Linux//"); mv "$1" "$newname"' sh {} \;
             #File 
               cd "$BINDIR" && find "./" -maxdepth 1 -type f | grep -v -E '\.jq$|\.txt$|\.upx$' | sort | xargs file > "$SYSTMP/x86_64_Linux_FILE"
             ...
             #BLAKE3SUM
               cd "$BINDIR" && find "./" -maxdepth 1 -type f | grep -v -E '\.jq$|\.txt$|\.upx$' | sort | xargs b3sum > "$SYSTMP/x86_64_Linux_BLAKE3SUM"
             ...
             #SHA256SUM
               cd "$BINDIR" && find "./" -maxdepth 1 -type f | grep -v -E '\.jq$|\.txt$|\.upx$' | sort | xargs sha256sum > "$SYSTMP/x86_64_Linux_SHA256SUM"
...
if [ "${UPX_PACK}" = "YES" ]; then
   ...
   #Pack:: https://github.com/upx/upx/blob/devel/doc/upx-doc.txt
     if command -v upx &> /dev/null && [ -d "${UPX_PACK_DIR}" ] && [ "$(find "${UPX_PACK_DIR}" -mindepth 1 -print -quit 2>/dev/null)" ]; then
       ...
       find . -type f -exec file -i '{}' \; | grep "application/.*executable" | cut -d':' -f1 | xargs realpath | sort -u -o "$SYSTMP/upx_input.txt"
       ...
       #Sync
        rclone copy "r2:/bin/x86_64_Linux/" "." --user-agent="$USER_AGENT" --buffer-size="10M" --s3-upload-concurrency="50" --s3-chunk-size="10M" --multi-thread-streams="50" --checkers="2000" --transfers="100" --retries="10" --check-first --checksum --copy-links --fast-list --progress && sleep 60
        rclone sync "." "r2:/bin/x86_64_Linux/" --user-agent="$USER_AGENT" --buffer-size="10M" --s3-upload-concurrency="50" --s3-chunk-size="10M" --multi-thread-streams="50" --checkers="2000" --transfers="100" --retries="10" --check-first --checksum --copy-links --fast-list --progress && sleep 60
     fi
   #Cleanup
     popd >/dev/null 2>&1
