...
if [ "$SKIP_BUILD" == "NO" ]; then
     ...
     export BIN="tracexec" #Name of final binary/pkg/cli, sometimes differs from $REPO
     export SOURCE_URL="https://github.com/kxxt/tracexec" #github/gitlab/homepage/etc for $BIN
     echo -e "\n\n [+] (Building | Fetching) $BIN :: $SOURCE_URL\n"
      #Build
       ...
       docker run --privileged --net="host" --name "alpine-builder" --pull="always" "azathothas/alpine-builder:latest" \
        ...
        #Build
         git clone --filter "blob:none" --quiet "https://github.com/kxxt/tracexec" && cd "./tracexec"
         echo -e "\n[+] Target: $RUST_TARGET\n"
         echo -e "\n[+] Flags: $RUSTFLAGS\n"
         ...
         cp "./target/$RUST_TARGET/release/tracexec" "/build-bins/tracexec"
        '
      #Copy 
       docker cp "alpine-builder:/build-bins/tracexec" "./tracexec"
       #Meta 
       file "./tracexec" && du -sh "./tracexec"
       cp "./tracexec" "$BINDIR/tracexec"
      #Delete Containers
