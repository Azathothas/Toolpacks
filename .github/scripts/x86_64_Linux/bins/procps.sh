    ...
    #procps : Utilities that give information about processes using the /proc filesystem
     ...
     #-------------------------------------------------------#    
      ##Build 
       ...
       #Strip
       find "$BASEUTILSDIR/procps" -type f -exec strip {} \; 2>/dev/null
      #-------------------------------------------------------#       
      ##Meta
       ...
       #Generate METADATA
       cd "$BASEUTILSDIR/procps" && find "./" -maxdepth 1 -type f | grep -v '.txt' | sort | xargs file > "$BASEUTILSDIR/procps/FILE.txt"
       cd "$BASEUTILSDIR/procps" && find "./" -maxdepth 1 -type f | grep -v '.txt' | sort | xargs b3sum > "$BASEUTILSDIR/procps/BLAKE3SUM.txt"
       cd "$BASEUTILSDIR/procps" && find "./" -maxdepth 1 -type f | grep -v '.txt' | sort | xargs sha256sum > "$BASEUTILSDIR/procps/SHA256SUM.txt"
       ...
       cd "$BASEUTILSDIR/procps" && rclone copy "." "r2:/bin/x86_64_Linux/Baseutils/procps/" --exclude="*.jq" --user-agent="$USER_AGENT" --s3-upload-concurrency="500" --s3-chunk-size="100M" --multi-thread-streams="500" --checkers="2000" --transfers="1000" --retries="10" --check-first --checksum --copy-links --fast-list --progress
       curl -qfsSL "https://pub.ajam.dev/utils/devscripts/jq/to_human_bytes.jq" -o "./to_human_bytes.jq"
       #List
       curl -qfsSL "https://pub.ajam.dev/repos/Azathothas/Toolpacks/.github/scripts/x86_64_Linux/bins/procps.yaml" -o "$TMP_METADIR/temp.yaml"
       ...
       rclone lsjson --fast-list "r2:/bin/x86_64_Linux/Baseutils/procps/"  --exclude="BLAKE3SUM" --exclude="*.7z" --exclude="*.json" --exclude="*.log" --exclude="*.md" --exclude="SHA256SUM" --exclude="*.txt" | \
       ...
       for BIN in $(cat "$TMP_METADIR/BINS.txt" | sed 's/"//g'); do
         ...
         #BSUM
          B3SUM="$(cat "$BASEUTILSDIR/procps/BLAKE3SUM.txt" | grep -iF "${BIN}" | awk '{print $1}' | sort  -u | head -n 1 | sed 's/"//g' | sed 's/^[ \t]*//;s/[ \t]*$//' | sed 's/["'\'']//g' | sed 's/`//g' | sed 's/|//g' | tr -d '[:space:]')" && export B3SUM="$B3SUM"
         ...
         #SHA256SUM
          SHA256="$(cat "$BASEUTILSDIR/procps/SHA256SUM.txt" | grep -iF "${BIN}" | awk '{print $1}' | sort  -u | head -n 1 | sed 's/"//g' | sed 's/^[ \t]*//;s/[ \t]*$//' | sed 's/["'\'']//g' | sed 's/`//g' | sed 's/|//g' | tr -d '[:space:]')" && export SHA256="$SHA256"
