...
if [ "$SKIP_BUILD" == "NO" ]; then
    #dotenv-linter: ⚡️Lightning-fast linter for .env files. Written in Rust 🦀
     export BIN="dotenv-linter" #Name of final binary/pkg/cli, sometimes differs from $REPO
     export SOURCE_URL="https://github.com/dotenv-linter/dotenv-linter" #github/gitlab/homepage/etc for $BIN
     echo -e "\n\n [+] (Building | Fetching) $BIN :: $SOURCE_URL\n"
      #Build 
       ...
       cargo build --target "$RUST_TARGET" --release --jobs="$(($(nproc)+1))" --keep-going ; cp "./target/$RUST_TARGET/release/dotenv-linter" "$BINDIR/dotenv-linter" ; popd > /dev/null 2>&1
fi
#-------------------------------------------------------#
