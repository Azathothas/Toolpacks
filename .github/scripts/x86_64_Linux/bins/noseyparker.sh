...
if [ "${SKIP_BUILD}" == "NO" ]; then
    #noseyparker: command-line program that finds secrets and sensitive information in textual data and Git history.
     export BIN="noseyparker" #Name of final binary/pkg/cli, sometimes differs from $REPO
     export SOURCE_URL="https://github.com/praetorian-inc/noseyparker" #github/gitlab/homepage/etc for $BIN
     echo -e "\n\n [+] (Building | Fetching) ${BIN} :: ${SOURCE_URL} [$(TZ='UTC' date +'%A, %Y-%m-%d (%I:%M:%S %p)') UTC]\n"
      #Build
       ...
       docker run --privileged --net="host" --name "alpine-builder" --pull="always" "azathothas/alpine-builder:latest" \
        ...
        #Build
         git clone --filter "blob:none" --quiet "https://github.com/praetorian-inc/noseyparker" && cd "./noseyparker"
         ...
         cp "./target/$RUST_TARGET/release/noseyparker-cli" "/build-bins/noseyparker"
        '  
      #Copy 
       docker cp "alpine-builder:/build-bins/noseyparker" "./noseyparker"
       #Meta 
       file "./noseyparker" && du -sh "./noseyparker"
       cp "./noseyparker" "$BINDIR/noseyparker"
      #Delete Containers
