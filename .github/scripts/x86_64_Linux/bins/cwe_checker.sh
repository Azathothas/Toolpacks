...
if [ "$SKIP_BUILD" == "NO" ]; then
    #cwe_checker: cwe_checker finds vulnerable patterns in binary executables
     export BIN="cwe_checker" #Name of final binary/pkg/cli, sometimes differs from $REPO
     export SOURCE_URL="https://github.com/fkie-cad/cwe_checker" #github/gitlab/homepage/etc for $BIN
     echo -e "\n\n [+] (Building | Fetching) $BIN :: $SOURCE_URL\n"
      #Build    
       pushd "$($TMPDIRS)" > /dev/null 2>&1 && git clone --quiet --filter "blob:none" "https://github.com/fkie-cad/cwe_checker" && cd "./cwe_checker"
       ...
       cargo build --target "$RUST_TARGET" --release --jobs="$(($(nproc)+1))" --keep-going ; cp "./target/$RUST_TARGET/release/cwe_checker" "$BINDIR/cwe_checker" ; popd > /dev/null 2>&1
fi
#-------------------------------------------------------#
