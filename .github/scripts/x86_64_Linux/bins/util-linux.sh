    ...
    #util-linux : random collection of Linux utilities
     ...
     #-------------------------------------------------------#    
      ##Build 
       ...
       #Strip
       find "$BASEUTILSDIR/util-linux" -type f -exec strip {} \; 2>/dev/null
      #-------------------------------------------------------#       
      ##Meta
       ...
       #Generate METADATA
       cd "$BASEUTILSDIR/util-linux" && find "./" -maxdepth 1 -type f | grep -v '.txt' | sort | xargs file > "$BASEUTILSDIR/util-linux/FILE.txt"
       cd "$BASEUTILSDIR/util-linux" && find "./" -maxdepth 1 -type f | grep -v '.txt' | sort | xargs b3sum > "$BASEUTILSDIR/util-linux/BLAKE3SUM.txt"
       cd "$BASEUTILSDIR/util-linux" && find "./" -maxdepth 1 -type f | grep -v '.txt' | sort | xargs sha256sum > "$BASEUTILSDIR/util-linux/SHA256SUM.txt"
       ...
       cd "$BASEUTILSDIR/util-linux" && rclone copy "." "r2:/bin/x86_64_Linux/Baseutils/util-linux/" --exclude="*.jq" --user-agent="$USER_AGENT" --s3-upload-concurrency="500" --s3-chunk-size="100M" --multi-thread-streams="500" --checkers="2000" --transfers="1000" --retries="10" --check-first --checksum --copy-links --fast-list --progress
       ...
       rclone lsjson --fast-list "r2:/bin/x86_64_Linux/Baseutils/util-linux/"  --exclude="BLAKE3SUM" --exclude="*.7z" --exclude="*.json" --exclude="*.log" --exclude="*.md" --exclude="SHA256SUM" --exclude="*.txt" | \
       ...
       for BIN in $(cat "$TMP_METADIR/BINS.txt" | sed 's/"//g'); do
         ...
         #BSUM
          B3SUM="$(cat "$BASEUTILSDIR/util-linux/BLAKE3SUM.txt" | grep -i "\b$BIN\b" | awk '{print $1}' | sort  -u | head -n 1 | sed 's/"//g' | sed 's/^[ \t]*//;s/[ \t]*$//' | sed 's/["'\'']//g' | sed 's/`//g' | sed 's/|//g' | tr -d '[:space:]')" && export B3SUM="$B3SUM"
         ...
         #SHA256SUM
          SHA256="$(cat "$BASEUTILSDIR/util-linux/SHA256SUM.txt" | grep -i "\b$BIN\b" | awk '{print $1}' | sort  -u | head -n 1 | sed 's/"//g' | sed 's/^[ \t]*//;s/[ \t]*$//' | sed 's/["'\'']//g' | sed 's/`//g' | sed 's/|//g' | tr -d '[:space:]')" && export SHA256="$SHA256"
