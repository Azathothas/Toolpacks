...
if [ "$SKIP_BUILD" == "NO" ]; then
     ...
     export BIN="elfcat" #Name of final binary/pkg/cli, sometimes differs from $REPO
     export SOURCE_URL="https://github.com/ruslashev/elfcat" #github/gitlab/homepage/etc for $BIN
     echo -e "\n\n [+] (Building | Fetching) $BIN :: $SOURCE_URL\n"
      #Build
       ...
       cargo build --target "$RUST_TARGET" --release --jobs="$(($(nproc)+1))" --keep-going ; cp "./target/$RUST_TARGET/release/elfcat" "$BINDIR/elfcat" ; popd >/dev/null 2>&1
fi
#-------------------------------------------------------#
