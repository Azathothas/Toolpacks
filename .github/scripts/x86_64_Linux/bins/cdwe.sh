...
if [ "$SKIP_BUILD" == "NO" ]; then
    #cdwe: ⚡️(cd with env) Is a configurable cd wrapper that lets you define your environment per directory.
     export BIN="cdwe"
     export SOURCE_URL="https://github.com/synoet/cdwe"
     echo -e "\n\n [+] (Building | Fetching) $BIN :: $SOURCE_URL\n"
      #Build
       ...
       docker run --privileged --net="host" --name "alpine-builder" --pull="always" "azathothas/alpine-builder:latest" \
        ...
        #Build
         git clone --filter "blob:none" --quiet "https://github.com/synoet/cdwe" && cd "./cdwe"
         echo -e "\n[+] Target: $RUST_TARGET\n"
         echo -e "\n[+] Flags: $RUSTFLAGS\n"
         sed "/^\[profile\.release\]/,/^$/d" -i "./Cargo.toml" ; echo -e "\n[profile.release]\nstrip = true\nopt-level = 3\nlto = true" >> "./Cargo.toml"
         rm rust-toolchain* 2>/dev/null
         cargo build --target "$RUST_TARGET" --release --jobs="$(($(nproc)+1))" --keep-going
         cp "./target/$RUST_TARGET/release/cdwe" "/build-bins/cdwe"
        '
      #Copy 
       docker cp "alpine-builder:/build-bins/cdwe" "./cdwe"
       #Meta 
       file "./cdwe" && du -sh "./cdwe" ; cp "./cdwe" "$BINDIR/cdwe"
      #Delete Containers
