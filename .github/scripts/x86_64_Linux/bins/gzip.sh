    ...
    #gzip : data compression program
     ...
     #-------------------------------------------------------#    
      ...
      ##Meta
       ...
       cd "$BASEUTILSDIR/gzip" && rclone copy "." "r2:/bin/x86_64_Linux/Baseutils/gzip/" --exclude="*.jq" --user-agent="$USER_AGENT" --s3-upload-concurrency="500" --s3-chunk-size="100M" --multi-thread-streams="500" --checkers="2000" --transfers="1000" --retries="10" --check-first --checksum --copy-links --fast-list --progress
       ...
       rclone lsjson --fast-list "r2:/bin/x86_64_Linux/Baseutils/gzip/"  --exclude="BLAKE3SUM" --exclude="*.7z" --exclude="*.json" --exclude="*.log" --exclude="*.md" --exclude="SHA256SUM" --exclude="*.txt" | \
