...
if [ "$SKIP_BUILD" == "NO" ]; then
    #wasminspect: An interactive debugger for WebAssembly
     export BIN="wasminspect" #Name of final binary/pkg/cli, sometimes differs from $REPO
     export SOURCE_URL="https://github.com/kateinoigakukun/wasminspect" #github/gitlab/homepage/etc for $BIN
     echo -e "\n\n [+] (Building | Fetching) $BIN :: $SOURCE_URL\n"
      #Build   
       ...
       cargo build --target "$RUST_TARGET" --release --jobs="$(($(nproc)+1))" --keep-going ; mv "./target/$RUST_TARGET/release/wasminspect" "$BINDIR/wasminspect" ; popd > /dev/null 2>&1
fi
#-------------------------------------------------------#
