...
#-------------------------------------------------------#
##Main
export SKIP_BUILD="YES" #https://github.com/queer/boxxy/issues/37
if [ "$SKIP_BUILD" == "NO" ]; then
      #boxxy : boxxy puts bad Linux applications in a box with only their files.
     export BIN="boxxy" #Name of final binary/pkg/cli, sometimes differs from $REPO
     export SOURCE_URL="https://github.com/queer/boxxy" #github/gitlab/homepage/etc for $BIN
     echo -e "\n\n [+] (Building | Fetching) $BIN :: $SOURCE_URL\n"
      #Build
       ...
       cargo build --target "$RUST_TARGET" --release --jobs="$(($(nproc)+1))" --keep-going ; cp "./target/$RUST_TARGET/release/boxxy" "$BINDIR/boxxy" ; popd > /dev/null 2>&1 
fi
#-------------------------------------------------------#
