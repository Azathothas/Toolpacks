...
if [ "$SKIP_BUILD" == "NO" ]; then
    #ubase : a collection of unportable tools, similar in spirit to util-linux but much simpler.
     export BIN="ubase" #Name of final binary/pkg/cli, sometimes differs from $REPO
     ...
     echo -e "\n\n [+] (Building | Fetching) $BIN :: $SOURCE_URL\n"
      #Build 
       ...
       #If no zig
       make CFLAGS="$CFLAGS ${ADDITIONAL_ARGS}" CXXFLAGS="$CFLAGS ${ADDITIONAL_ARGS}" LDFLAGS="$LDFLAGS ${ADDITIONAL_ARGS}" --jobs="$(($(nproc)+1))" --keep-going ubase-box
       strip "./ubase-box" ; file "./ubase-box" && du -sh "./ubase-box"
       cp "./ubase-box" "$BINDIR/ubase-box"
