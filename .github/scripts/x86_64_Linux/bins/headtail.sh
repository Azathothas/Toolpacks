...
if [ "$SKIP_BUILD" == "NO" ]; then
    #headtail: head and tail simultaneously 
     export BIN="headtail" #Name of final binary/pkg/cli, sometimes differs from $REPO
     export SOURCE_URL="https://github.com/CleanCut/headtail" #github/gitlab/homepage/etc for $BIN
     echo -e "\n\n [+] (Building | Fetching) $BIN :: $SOURCE_URL\n"
      #Build    
       pushd "$($TMPDIRS)" >/dev/null 2>&1 && git clone --quiet --filter "blob:none" "https://github.com/CleanCut/headtail" && cd "./headtail"
       ...
       cargo build --target "$RUST_TARGET" --release --jobs="$(($(nproc)+1))" --keep-going ; cp "./target/$RUST_TARGET/release/headtail" "$BINDIR/headtail" ; popd >/dev/null 2>&1
fi
#-------------------------------------------------------#
