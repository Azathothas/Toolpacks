...
if [ "$SKIP_BUILD" == "NO" ]; then
     ...
     export BIN="wireguard-rs" #Name of final binary/pkg/cli, sometimes differs from $REPO
     export SOURCE_URL="https://git.zx2c4.com/wireguard-rs" #github/gitlab/homepage/etc for $BIN
     echo -e "\n\n [+] (Building | Fetching) $BIN :: $SOURCE_URL\n"
      #Build 
       ...
       cargo build --target "$RUST_TARGET" --release --jobs="$(($(nproc)+1))" --keep-going ; cp "./target/$RUST_TARGET/release/wireguard-rs" "$BINDIR/wireguard-rs"
fi
#-------------------------------------------------------#
