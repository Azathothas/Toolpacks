...
if [ "$SKIP_BUILD" == "NO" ]; then
     ...
     export BIN="gip" #Name of final binary/pkg/cli, sometimes differs from $REPO
     export SOURCE_URL="https://github.com/dalance/gip" #github/gitlab/homepage/etc for $BIN
     echo -e "\n\n [+] (Building | Fetching) $BIN :: $SOURCE_URL\n"
      #Build  
       ...
       cargo build --target "$RUST_TARGET" --release --jobs="$(($(nproc)+1))" --keep-going ; mv "./target/$RUST_TARGET/release/gip" "$BINDIR/gip" ; popd > /dev/null 2>&1
fi
#-------------------------------------------------------#
