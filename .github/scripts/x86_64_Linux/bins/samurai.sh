...
if [ "$SKIP_BUILD" == "NO" ]; then
    #samurai :  ninja-compatible build tool written in C 
     export BIN="samurai" #Name of final binary/pkg/cli, sometimes differs from $REPO
     ...
     echo -e "\n\n [+] (Building | Fetching) $BIN :: $SOURCE_URL\n"
      #Build 
       ...
       #If no zig
       make CFLAGS="$CFLAGS ${ADDITIONAL_ARGS}" CXXFLAGS="$CFLAGS ${ADDITIONAL_ARGS}" LDFLAGS="$LDFLAGS ${ADDITIONAL_ARGS}" --jobs="$(($(nproc)+1))" --keep-going samurai
