...
if [ "$SKIP_BUILD" == "NO" ]; then
    #kalc: a complex numbers, 2d/3d graphing, arbitrary precision, vector/matrix, cli calculator with real-time output and support for units
     export BIN="kalc"
     export SOURCE_URL="https://github.com/bgkillas/kalc"
     echo -e "\n\n [+] (Building | Fetching) $BIN :: $SOURCE_URL\n"
      #Build
       ...
       docker run --privileged --net="host" --name "alpine-builder" --pull="always" "azathothas/alpine-builder:latest" \
        ...
        #Build
         git clone --filter "blob:none" --quiet "https://github.com/bgkillas/kalc" && cd "./kalc"
         echo -e "\n[+] Target: $RUST_TARGET\n"
         echo -e "\n[+] Flags: $RUSTFLAGS\n"
         sed "/^\[profile\.release\]/,/^$/d" -i "./Cargo.toml" ; echo -e "\n[profile.release]\nstrip = true\nopt-level = 3\nlto = true" >> "./Cargo.toml"
         rm rust-toolchain* 2>/dev/null
         cargo build --target "$RUST_TARGET" --release --jobs="$(($(nproc)+1))" --keep-going
         cp "./target/$RUST_TARGET/release/kalc" "/build-bins/kalc"
        '
      #Copy 
       docker cp "alpine-builder:/build-bins/kalc" "./kalc"
       #Meta 
       file "./kalc" && du -sh "./kalc" ; cp "./kalc" "$BINDIR/kalc"
      #Delete Containers
