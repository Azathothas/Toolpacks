...
if [ "${SKIP_BUILD}" == "NO" ]; then
    #soar: A highly experimental package manager
     export BIN="soar"
     export SOURCE_URL="https://github.com/QaidVoid/soar"
     echo -e "\n\n [+] (Building | Fetching) ${BIN} :: ${SOURCE_URL} [$(TZ='UTC' date +'%A, %Y-%m-%d (%I:%M:%S %p)') UTC]\n"
      #Build
       ...
       docker run --privileged --net="host" --name "alpine-builder" --pull="always" "azathothas/alpine-builder:latest" \
        ...
        #Build
         git clone --filter "blob:none" --quiet "https://github.com/QaidVoid/soar" && cd "./soar"
         echo -e "\n[+] Target: ${RUST_TARGET}\n"
         echo -e "\n[+] Flags: ${RUSTFLAGS}\n"
