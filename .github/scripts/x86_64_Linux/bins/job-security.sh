...
if [ "$SKIP_BUILD" == "NO" ]; then
     ...
     export BIN="job-security" #Name of final binary/pkg/cli, sometimes differs from $REPO
     export SOURCE_URL="https://github.com/yshui/job-security" #github/gitlab/homepage/etc for $BIN
     echo -e "\n\n [+] (Building | Fetching) $BIN :: $SOURCE_URL\n"
      #Build
       ...
       docker run --privileged --net="host" --name "alpine-builder" "azathothas/alpine-builder:latest" \
        ...
        #Build
         git clone --filter "blob:none" --quiet "https://github.com/yshui/job-security" && cd "./job-security"
         echo -e "\n[+] Target: $RUST_TARGET\n"
         echo -e "\n[+] Flags: $RUSTFLAGS\n"
         ...
         cp "./target/$RUST_TARGET/release/jobs" "/build-bins/jobs"
        '
      #Copy 
       docker cp "alpine-builder:/build-bins/jobs" "./jobs"
       #Meta 
       file "./jobs" && du -sh "./jobs"
       cp "./jobs" "$BINDIR/jobs"
      #Delete Containers
