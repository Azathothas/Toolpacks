...
if [ "$SKIP_BUILD" == "NO" ]; then
    #ds (dirstat-rs) : (fastest?) disk usage cli, similar to windirstat.
     export BIN="ds" #Name of final binary/pkg/cli, sometimes differs from $REPO
     export SOURCE_URL="https://github.com/scullionw/dirstat-rs" #github/gitlab/homepage/etc for $BIN
     echo -e "\n\n [+] (Building | Fetching) $BIN :: $SOURCE_URL\n"
      #Build  
       ...
       cargo build --target "$RUST_TARGET" --release --jobs="$(($(nproc)+1))" --keep-going; cp "./target/$RUST_TARGET/release/ds" "$BINDIR/ds" ; popd > /dev/null 2>&1
fi
#-------------------------------------------------------#
