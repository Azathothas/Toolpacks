...
if [ "$SKIP_BUILD" == "NO" ]; then
   #killport : A command-line tool to easily kill processes running on a specified port.
     export BIN="killport" #Name of final binary/pkg/cli, sometimes differs from $REPO
     export SOURCE_URL="https://github.com/jkfran/killport" #github/gitlab/homepage/etc for $BIN
     echo -e "\n\n [+] (Building | Fetching) $BIN :: $SOURCE_URL\n"
      #Build
       pushd "$($TMPDIRS)" >/dev/null 2>&1 && git clone --quiet --filter "blob:none" "https://github.com/jkfran/killport" && cd "./killport"
