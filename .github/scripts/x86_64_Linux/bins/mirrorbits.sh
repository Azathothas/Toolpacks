...
if [ "${SKIP_BUILD}" == "NO" ]; then
    #mirrorbits : geographical download redirector written in Go for distributing files efficiently across a set of mirrors. 
     export BIN="mirrorbits" #Name of final binary/pkg/cli, sometimes differs from $REPO
     export SOURCE_URL="https://github.com/etix/mirrorbits" #github/gitlab/homepage/etc for $BIN
     echo -e "\n\n [+] (Building | Fetching) ${BIN} :: ${SOURCE_URL} [$(TZ='UTC' date +'%A, %Y-%m-%d (%I:%M:%S %p)') UTC]\n"
      #Build (alpine-musl)
       ...
       docker run --privileged --net="host" --name "alpine-builder" --pull="always" "azathothas/alpine-builder:latest" \
        ...
        '
      #Copy
       docker cp "alpine-builder:/build-bins/mirrorbits" "./mirrorbits"
       #Meta 
       file "./mirrorbits" && du -sh "./mirrorbits" ; cp "./mirrorbits" "$BINDIR/mirrorbits"
      #Delete Containers
