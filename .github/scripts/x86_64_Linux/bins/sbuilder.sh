...
if [ "${SKIP_BUILD}" == "NO" ]; then
    #sbuilder: Utilities for linting and building SBUILD packages
     export BIN="sbuilder"
     export SOURCE_URL="https://github.com/pkgforge/sbuilder"
     echo -e "\n\n [+] (Building | Fetching) ${BIN} :: ${SOURCE_URL} [$(TZ='UTC' date +'%A, %Y-%m-%d (%I:%M:%S %p)') UTC]\n"
      #Build
       ...
       docker run --privileged --net="host" --name "alpine-builder" --pull="always" "azathothas/alpine-builder:latest" \
        ...
        #Build Linter
         pushd "$(mktemp -d)" >/dev/null 2>&1 && git clone --filter "blob:none" --quiet "https://github.com/pkgforge/sbuilder" && cd "./sbuilder/sbuild-linter"
         echo -e "\n[+] Target: ${RUST_TARGET}\n"
         echo -e "\n[+] Flags: ${RUSTFLAGS}\n"
         sed "/^\[profile\.release\]/,/^$/d" -i "./Cargo.toml" ; echo -e "\n[profile.release]\nstrip = true\nopt-level = 3\nlto = true" >> "./Cargo.toml"
         rm rust-toolchain* 2>/dev/null
         cargo build --target "${RUST_TARGET}" --release --jobs="$(($(nproc)+1))" --keep-going
         find "./target/${RUST_TARGET}/release" -maxdepth 1 -type f -exec file -i "{}" \; | grep "application/.*executable" | cut -d":" -f1 | xargs realpath | xargs -I {} rsync -achvL "{}" "/build-bins/sbuild-linter"
        #Build Builder
         #pushd "$(mktemp -d)" >/dev/null 2>&1 && git clone --filter "blob:none" --quiet "https://github.com/pkgforge/sbuilder" && cd "./sbuilder/sbuild-linter"
         #echo -e "\n[+] Target: ${RUST_TARGET}\n"
         #echo -e "\n[+] Flags: ${RUSTFLAGS}\n"
         ...
         #rm rust-toolchain* 2>/dev/null
         #cargo build --target "${RUST_TARGET}" --release --jobs="$(($(nproc)+1))" --keep-going
         #find "./target/${RUST_TARGET}/release" -maxdepth 1 -type f -exec file -i "{}" \; | grep "application/.*executable" | cut -d":" -f1 | xargs realpath | xargs -I {} rsync -achvL "{}" "/build-bins/sbuild-builder"
        #Strip
