...
if [ "$SKIP_BUILD" == "NO" ]; then
    #geonet: Network tools that run from multiple geographic locations using the GeoNet API. 
     export BIN="geonet-rs" #Name of final binary/pkg/cli, sometimes differs from $REPO
     export SOURCE_URL="https://gitlab.com/shodan-public/geonet-rs" #github/gitlab/homepage/etc for $BIN
     echo -e "\n\n [+] (Building | Fetching) $BIN :: $SOURCE_URL\n"
      #Build
       ...
       docker run --privileged --net="host" --name "alpine-builder" --pull="always" "azathothas/alpine-builder:latest" \
        ...
        #Build
         git clone --filter "blob:none" --quiet "https://gitlab.com/shodan-public/geonet-rs" && cd "./geonet-rs"
         echo -e "\n[+] Target: $RUST_TARGET\n"
         echo -e "\n[+] Flags: $RUSTFLAGS\n"
         ...
         cp "./target/$RUST_TARGET/release/geodns" "/build-bins/geodns"
         #geoping: ping an IP/ hostname from multiple locations
         cp "./target/$RUST_TARGET/release/geoping" "/build-bins/geoping"
        '
      #Copy 
       docker cp "alpine-builder:/build-bins/geodns" "./geodns"
       docker cp "alpine-builder:/build-bins/geoping" "./geoping"
       #Meta 
       file "./geodns" && du -sh "./geodns" ; cp "./geodns" "$BINDIR/geodns"
       file "./geoping" && du -sh "./geoping" ; cp "./geoping" "$BINDIR/geoping"
      #Delete Containers
