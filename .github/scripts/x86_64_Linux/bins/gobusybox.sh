    ...
    #gobusybox : Tools for compiling many Go commands into one binary to save space
     ...
     echo -e "\n\n [+] (Building | Fetching) $BIN :: $SOURCE_URL\n"
      #Build  
       ...
       GOOS="linux" GOARCH="amd64" CGO_ENABLED="0" go build -v -ldflags="-buildid= -s -w -extldflags '-static'" "./cmd/goanywhere" ; cp "./goanywhere" "$BINDIR/goanywhere"
       #makebb : Compiles many Go commands into one bb-style binary.
       GOOS="linux" GOARCH="amd64" CGO_ENABLED="0" go build -v -ldflags="-buildid= -s -w -extldflags '-static'" "./cmd/makebb" ; cp "./makebb" "$BINDIR/makebb"
       #makebbmain : adds u-root command package imports to an existing main() template file.
       GOOS="linux" GOARCH="amd64" CGO_ENABLED="0" go build -v -ldflags="-buildid= -s -w -extldflags '-static'" "./cmd/makebbmain" ; cp "./makebbmain" "$BINDIR/makebbmain"
