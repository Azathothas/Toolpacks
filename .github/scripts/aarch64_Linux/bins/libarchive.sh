    ...
    #libarchive : Multi-format archive and compression Tools
     ...
     #-------------------------------------------------------#    
      ##Build (nix)
       ...
       #Strip
       find "$BASEUTILSDIR/libarchive" -type f -exec strip {} \; 2>/dev/null
      ...
      ##Meta
       ...
       #Generate METADATA
       cd "$BASEUTILSDIR/libarchive" && find "./" -maxdepth 1 -type f | grep -v '.txt' | sort | xargs file > "$BASEUTILSDIR/libarchive/FILE.txt"
       cd "$BASEUTILSDIR/libarchive" && find "./" -maxdepth 1 -type f | grep -v '.txt' | sort | xargs b3sum > "$BASEUTILSDIR/libarchive/BLAKE3SUM.txt"
       cd "$BASEUTILSDIR/libarchive" && find "./" -maxdepth 1 -type f | grep -v '.txt' | sort | xargs sha256sum > "$BASEUTILSDIR/libarchive/SHA256SUM.txt"
       dust --depth 1 --only-file --no-percent-bars --no-colors --ignore_hidden --reverse --number-of-lines 99999999 --invert-filter "\.7z$|\.jq$|\.md$|\.rar$|\.tar$|\.tmp$|\.txt$|\.zip$" "$BASEUTILSDIR/libarchive" | tee "$BASEUTILSDIR/libarchive/SIZE.txt"
       #rClone
       ...
       #List
       curl -qfsSL "https://pub.ajam.dev/repos/Azathothas/Toolpacks/.github/scripts/aarch64_Linux/bins/libarchive.yaml" -o "$TMP_METADIR/temp.yaml"
       ...
       rclone lsjson --fast-list "r2:/bin/aarch64_arm64_Linux/Baseutils/libarchive/"  --exclude="BLAKE3SUM" --exclude="*.7z" --exclude="*.json" --exclude="*.log" --exclude="*.md" --exclude="SHA256SUM" --exclude="*.txt" | \
       jq --arg DESCRIPTION "$DESCRIPTION" --arg EXTRA_BINS "$EXTRA_BINS" --arg WEB_URL "$WEB_URL" --arg REPO_URL "$REPO_URL" -r 'include "./to_human_bytes" ; .[] | select(.Size != 0 and .Size != -1 and (.Name | test("\\.(7z|bz2|gz|json|md|rar|tar|tgz|tmp|txt|zip)$") | not)) | {name: (.Name), description: $DESCRIPTION, download_url: "https://bin.ajam.dev/aarch64_arm64_Linux/Baseutils/libarchive/\(.Path)", size: (.Size | tonumber | bytes), build_date: (.ModTime | split(".")[0]), repo_url: $REPO_URL, web_url: $WEB_URL, extra_bins: $EXTRA_BINS}' | jq -s 'sort_by(.name)' > "$TMP_METADIR/INFO.json"
       ...
       for BIN in $(cat "$TMP_METADIR/BINS.txt" | sed 's/"//g'); do
         ...
         #BSUM
          B3SUM="$(cat "$BASEUTILSDIR/libarchive/BLAKE3SUM.txt" | grep -iF "${BIN}" | awk '{print $1}' | sort  -u | head -n 1 | sed 's/"//g' | sed 's/^[ \t]*//;s/[ \t]*$//' | sed 's/["'\'']//g' | sed 's/`//g' | sed 's/|//g' | tr -d '[:space:]')" && export B3SUM="$B3SUM"
         ...
         #SHA256SUM
          SHA256="$(cat "$BASEUTILSDIR/libarchive/SHA256SUM.txt" | grep -iF "${BIN}" | awk '{print $1}' | sort  -u | head -n 1 | sed 's/"//g' | sed 's/^[ \t]*//;s/[ \t]*$//' | sed 's/["'\'']//g' | sed 's/`//g' | sed 's/|//g' | tr -d '[:space:]')" && export SHA256="$SHA256"
