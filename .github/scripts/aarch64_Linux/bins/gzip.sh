    ...
    #gzip : data compression program
     ...
     #-------------------------------------------------------#    
      ##Build 
       ...
       #Strip
       find "$BASEUTILSDIR/gzip" -type f -exec strip {} \; 2>/dev/null
      #-------------------------------------------------------#       
      ##Meta
       ...
       #Generate METADATA
       cd "$BASEUTILSDIR/gzip" && find "./" -maxdepth 1 -type f | grep -v '.txt' | sort | xargs file > "$BASEUTILSDIR/gzip/FILE.txt"
       cd "$BASEUTILSDIR/gzip" && find "./" -maxdepth 1 -type f | grep -v '.txt' | sort | xargs b3sum > "$BASEUTILSDIR/gzip/BLAKE3SUM.txt"
       cd "$BASEUTILSDIR/gzip" && find "./" -maxdepth 1 -type f | grep -v '.txt' | sort | xargs sha256sum > "$BASEUTILSDIR/gzip/SHA256SUM.txt"
       dust --depth 1 --only-file --no-percent-bars --no-colors --ignore_hidden --reverse --number-of-lines 99999999 --invert-filter "\.7z$|\.jq$|\.md$|\.rar$|\.tar$|\.tmp$|\.txt$|\.zip$" "$BASEUTILSDIR/gzip" | tee "$BASEUTILSDIR/gzip/SIZE.txt"
       #rClone
       TMP_METADIR="$(mktemp -d)" && export TMP_METADIR="$TMP_METADIR"
       cd "$BASEUTILSDIR/gzip" && rclone copy "." "r2:/bin/aarch64_arm64_Linux/Baseutils/gzip/" --exclude="*.jq" --user-agent="$USER_AGENT" --s3-upload-concurrency="500" --s3-chunk-size="100M" --multi-thread-streams="500" --checkers="2000" --transfers="1000" --retries="10" --check-first --checksum --copy-links --fast-list --progress
       ...
       rclone lsjson --fast-list "r2:/bin/aarch64_arm64_Linux/Baseutils/gzip/"  --exclude="BLAKE3SUM" --exclude="*.7z" --exclude="*.json" --exclude="*.log" --exclude="*.md" --exclude="SHA256SUM" --exclude="*.txt" | \
       ...
       for BIN in $(cat "$TMP_METADIR/BINS.txt" | sed 's/"//g'); do
         ...
         #BSUM
          B3SUM="$(cat "$BASEUTILSDIR/gzip/BLAKE3SUM.txt" | grep -iF "${BIN}" | awk '{print $1}' | sort  -u | head -n 1 | sed 's/"//g' | sed 's/^[ \t]*//;s/[ \t]*$//' | sed 's/["'\'']//g' | sed 's/`//g' | sed 's/|//g' | tr -d '[:space:]')" && export B3SUM="$B3SUM"
         ...
         #SHA256SUM
          SHA256="$(cat "$BASEUTILSDIR/gzip/SHA256SUM.txt" | grep -iF "${BIN}" | awk '{print $1}' | sort  -u | head -n 1 | sed 's/"//g' | sed 's/^[ \t]*//;s/[ \t]*$//' | sed 's/["'\'']//g' | sed 's/`//g' | sed 's/|//g' | tr -d '[:space:]')" && export SHA256="$SHA256"
