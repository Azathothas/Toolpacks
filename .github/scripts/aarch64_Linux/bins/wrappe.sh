...
if [ "${SKIP_BUILD}" == "NO" ]; then
    #wrappe: Packer for creating self-contained single-binary applications from executables and directories
     export BIN="wrappe"
     export SOURCE_URL="https://github.com/Systemcluster/wrappe"
     echo -e "\n\n [+] (Building | Fetching) ${BIN} :: ${SOURCE_URL} [$(TZ='UTC' date +'%A, %Y-%m-%d (%I:%M:%S %p)') UTC]\n"
      #Build
       ...
       docker run --privileged --net="host" --name "alpine-builder" --pull="always" "azathothas/alpine-builder:latest" \
        ...
        #Build
         git clone --filter "blob:none" --quiet "https://github.com/Systemcluster/wrappe" && cd "./wrappe"
         echo -e "\n[+] Target: ${RUST_TARGET}\n"
         echo -e "\n[+] Flags: ${RUSTFLAGS}\n"
