...
if [ "$SKIP_BUILD" == "NO" ]; then
   #jj : A Git-compatible DVCS that is both simple and powerful
     export BIN="jj" #Name of final binary/pkg/cli, sometimes differs from $REPO
     export SOURCE_URL="https://github.com/martinvonz/jj" #github/gitlab/homepage/etc for $BIN
     echo -e "\n\n [+] (Building | Fetching) $BIN :: $SOURCE_URL\n"
      #Build
       pushd "$($TMPDIRS)" >/dev/null 2>&1 && git clone --quiet --filter "blob:none" "https://github.com/martinvonz/jj" && cd "./jj"
       ...
       cargo build --target "$RUST_TARGET" --release --jobs="$(($(nproc)+1))" --keep-going ; cp "./target/$RUST_TARGET/release/jj" "$BINDIR/jj" ; popd >/dev/null 2>&1
fi
#-------------------------------------------------------#
