...
if [ "${SKIP_BUILD}" == "NO" ]; then
    #php : Standalone PHP binary
     ...
     echo -e "\n\n [+] (Building | Fetching) ${BIN} :: ${SOURCE_URL} [$(TZ='UTC' date +'%A, %Y-%m-%d (%I:%M:%S %p)') UTC]\n"
      #Fetch (cli)
       pushd "$($TMPDIRS)" >/dev/null 2>&1 && curl -A "$USER_AGENT" -qfsSLJO "https://dl.static-php.dev/static-php-cli/bulk/$(curl -qfsSL "https://dl.static-php.dev/static-php-cli/bulk/" | grep -oP 'href="\K[^"]*\.tar\.gz' | grep -i 'linux' | grep -i 'aarch64' | grep -i 'cli' | sort --version-sort | tail -n 1)"
       ...
       popd >/dev/null 2>&1
      #Fetch (fpm)
       pushd "$($TMPDIRS)" >/dev/null 2>&1 && curl -A "$USER_AGENT" -qfsSLJO "https://dl.static-php.dev/static-php-cli/bulk/$(curl -qfsSL "https://dl.static-php.dev/static-php-cli/bulk/" | grep -oP 'href="\K[^"]*\.tar\.gz' | grep -i 'linux' | grep -i 'aarch64' | grep -i fpm | sort --version-sort | tail -n 1)"
       find . -type f -name '*.tar.gz' -exec tar -xzvf {} \; && find . -type f -name '*.tar.gz' -exec rm -rf {} \;
       ...
       popd >/dev/null 2>&1
      #Fetch (micro)
       pushd "$($TMPDIRS)" >/dev/null 2>&1 && curl -A "$USER_AGENT" -qfsSLJO "https://dl.static-php.dev/static-php-cli/bulk/$(curl -qfsSL "https://dl.static-php.dev/static-php-cli/bulk/" | grep -oP 'href="\K[^"]*\.tar\.gz' | grep -i 'linux' | grep -i 'aarch64' | grep -i micro | sort --version-sort | tail -n 1)"
       find . -type f -name '*.tar.gz' -exec tar -xzvf {} \; && find . -type f -name '*.tar.gz' -exec rm -rf {} \;
