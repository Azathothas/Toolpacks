...
if [ "$SKIP_BUILD" == "NO" ]; then
   #xplr : A hackable, minimal, fast TUI file explorer
     ...
     echo -e "\n\n [+] (Building | Fetching) $BIN :: $SOURCE_URL\n"
      #Build
       pushd "$($TMPDIRS)" >/dev/null 2>&1 && git clone --quiet --filter "blob:none" "https://github.com/sayanarijit/xplr" && cd "./xplr"
       ...
       cargo build --target "$RUST_TARGET" --release --jobs="$(($(nproc)+1))" --keep-going ; cp "./target/$RUST_TARGET/release/xplr" "$BINDIR/xplr" ; popd >/dev/null 2>&1
fi
#-------------------------------------------------------#
