...
if [ "${SKIP_BUILD}" == "NO" ]; then
     ...
     echo -e "\n\n [+] (Building | Fetching) ${BIN} :: ${SOURCE_URL} [$(TZ='UTC' date +'%A, %Y-%m-%d (%I:%M:%S %p)') UTC]\n"
      #Build 
       pushd "$($TMPDIRS)" >/dev/null 2>&1 && git clone --quiet --filter "blob:none" "https://github.com/sharkdp/bat" && cd "./bat"
       ...
       cp "./target/$RUST_TARGET/release/bat" "$BINDIR/bat"
       cp "./target/$RUST_TARGET/release/bat" "$BINDIR/batcat"
       popd >/dev/null 2>&1
fi
#-------------------------------------------------------#
