...
if [ "$SKIP_BUILD" == "NO" ]; then
     ...
     echo -e "\n\n [+] (Building | Fetching) $BIN :: $SOURCE_URL\n"
      #Build 
       ...
       cargo build --target "$RUST_TARGET" --release --jobs="$(($(nproc)+1))" --keep-going ; cp "./target/$RUST_TARGET/release/binary-security-check" "$BINDIR/binary-security-check" ; popd >/dev/null 2>&1
fi
#-------------------------------------------------------#
