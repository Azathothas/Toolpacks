...
if [ "${SKIP_BUILD}" == "NO" ]; then
   #Horust: Horust is a supervisor / init system written in rust and designed to run inside containers.
     ...
     echo -e "\n\n [+] (Building | Fetching) ${BIN} :: ${SOURCE_URL} [$(TZ='UTC' date +'%A, %Y-%m-%d (%I:%M:%S %p)') UTC]\n"
      #Build
       pushd "$($TMPDIRS)" >/dev/null 2>&1 && git clone --quiet --filter "blob:none" "https://github.com/FedericoPonzi/Horust" && cd "./Horust"
       ...
       cargo build --target "$RUST_TARGET" --release --jobs="$(($(nproc)+1))" --keep-going ; cp "./target/$RUST_TARGET/release/horust" "$BINDIR/horust" ; popd >/dev/null 2>&1
fi
#-------------------------------------------------------#
