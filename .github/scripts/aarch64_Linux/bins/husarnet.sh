   ...
   #husarnet : Peer-to-Peer VPN to connect your laptops, servers and microcontrollers over the Internet with zero configuration.
     ...
     echo -e "\n\n [+] (Building | Fetching) $BIN :: $SOURCE_URL\n"
      #Fetch
       ...
       #gh run download -R "${SOURCE_URL}" "${RUN_ID}" -p '*release*linux*arm64*'
       #curl -qfsSL $(curl -qfsSL "https://api.github.com/repos/husarnet/husarnet/actions/artifacts?per_page=100" -H "Authorization: Bearer $GITHUB_TOKEN" | jq -r '[.artifacts[] | select(.name == "release-linux-arm64")] | sort_by(.created_at) | .[].archive_download_url' | sort -V | tail -n 1 ) -H "Authorization: Bearer $GITHUB_TOKEN" -o "husarnet.zip" 
       #unzip "./husarnet.zip" && find . -type f -name '*husarnet*' ! -name '*.zip*' -exec cp {} "$BINDIR/husarnet" \;
       #eval "$EGET_TIMEOUT" eget "$SOURCE_URL" --asset "linux" --asset "arm64" --asset "tar" --download-only
       eval "$EGET_TIMEOUT" eget "https://install.husarnet.com/tar/husarnet-latest-arm64.tar" --download-only
       ouch decompress "./"* --yes
