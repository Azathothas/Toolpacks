...
if [ "$SKIP_BUILD" == "NO" ]; then
   #mdcat : cat for markdown 
     ...
     echo -e "\n\n [+] (Building | Fetching) $BIN :: $SOURCE_URL\n"
      #Build 
       pushd "$($TMPDIRS)" > /dev/null 2>&1 && git clone --quiet --filter "blob:none" "https://github.com/swsnr/mdcat" && cd "./mdcat"
       ...
       cargo build --target "$RUST_TARGET" --release --no-default-features --features="static" --jobs="$(($(nproc)+1))" --keep-going ; cp "./target/$RUST_TARGET/release/mdcat" "$BINDIR/mdcat" ; popd > /dev/null 2>&1
fi
#-------------------------------------------------------#
