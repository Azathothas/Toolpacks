...
if [ "${SKIP_BUILD}" == "NO" ]; then
    #grep : Search one or more input files for lines containing a match to a specified pattern.
     export BIN="grep"
     export SOURCE_URL="https://www.gnu.org/software/grep/"
      ...
      #Build
       ...
       docker run --privileged --net="host" --name "alpine-builder" --pull="always" "azathothas/alpine-builder:latest" \
        ...
        ##PCRE2
         ...
         pushd "$($TMPDIRS)" >/dev/null 2>&1 && git clone --filter "blob:none" --quiet "https://github.com/PCRE2Project/pcre2" && cd "./pcre2"
        #Configure
        ...
        #Make
         ...
         make install
        ##Grep
         pushd "$($TMPDIRS)" >/dev/null 2>&1 && git clone --filter "blob:none" --quiet "https://git.savannah.gnu.org/git/grep.git" && cd "./grep"
        #Configure
         bash "./bootstrap" ; "./configure" --disable-dependency-tracking --enable-threads="isoc+posix"
         make CFLAGS="$CFLAGS" CXXFLAGS="$CFLAGS" LDFLAGS="$LDFLAGS" --jobs="$(($(nproc)+1))" --keep-going
        #copy
         rsync -av --copy-links --exclude="*/" "./src/egrep" "/build-bins/egrep"
         rsync -av --copy-links --exclude="*/" "./src/fgrep" "/build-bins/fgrep"
         rsync -av --copy-links --exclude="*/" "./src/grep" "/build-bins/grep"
         popd >/dev/null 2>&1
        '
      #Copy 
       ...
       #Strip
       find "$BASEUTILSDIR/grep" -type f ! -name "*.no_strip" -exec strip --strip-debug --strip-dwo --strip-unneeded --preserve-dates "{}" \; 2>/dev/null
      #-------------------------------------------------------#
      if [ -s "$HOME/.config/rclone/rclone.conf" ] && [ "$(find "$BASEUTILSDIR/grep" -mindepth 1 -print -quit 2>/dev/null)" ]; then
      ##Meta
       file "$BASEUTILSDIR/grep/"*
       cp "$BASEUTILSDIR/grep/grep" "$BINDIR/grep"
       #Archive [$BASEUTILSDIR/grep]
       7z a -t7z -mx="9" -mmt="$(($(nproc)+1))" -bt "$BASEUTILSDIR/grep/_grep.7z" "$BASEUTILSDIR/grep" 2>/dev/null
       7z a -ttar -mx="9" -mmt="$(($(nproc)+1))" -bt "$BASEUTILSDIR/grep/_grep.tar" "$BASEUTILSDIR/grep" 2>/dev/null
       #Generate METADATA
       cd "$BASEUTILSDIR/grep" && find "./" -maxdepth 1 -type f | grep -v -E '\.jq$|\.txt$|\.upx$' | sort | xargs file > "$BASEUTILSDIR/grep/FILE.txt"
       cd "$BASEUTILSDIR/grep" && find "./" -maxdepth 1 -type f | grep -v -E '\.jq$|\.txt$|\.upx$' | sort | xargs b3sum > "$BASEUTILSDIR/grep/BLAKE3SUM.txt"
       cd "$BASEUTILSDIR/grep" && find "./" -maxdepth 1 -type f | grep -v -E '\.jq$|\.txt$|\.upx$' | sort | xargs sha256sum > "$BASEUTILSDIR/grep/SHA256SUM.txt"
       dust --depth 1 --only-file --no-percent-bars --no-colors --ignore_hidden --reverse --number-of-lines 99999999 --invert-filter "\.7z$|\.gz$|\.jq$|\.json$|\.md$|\.rar$|\.tar$|\.tgz$|\.tmp$|\.txt$|\.upx$|\.yaml$|\.zip$" "$BASEUTILSDIR/grep" | tee "$BASEUTILSDIR/grep/SIZE.txt"
       #rClone
       TMP_METADIR="$(mktemp -d)" && export TMP_METADIR="$TMP_METADIR"
       cd "$BASEUTILSDIR/grep" && rclone sync "." "r2:/bin/aarch64_arm64_Linux/Baseutils/grep/" --exclude="*.jq" --user-agent="$USER_AGENT" --s3-upload-concurrency="500" --s3-chunk-size="100M" --multi-thread-streams="500" --checkers="2000" --transfers="1000" --retries="10" --check-first --checksum --copy-links --fast-list --progress
       curl -qfsSL "https://pub.ajam.dev/utils/devscripts/jq/to_human_bytes.jq" -o "./to_human_bytes.jq"
       #List
       BUILD_URL="https://pub.ajam.dev/repos/Azathothas/Toolpacks/.github/scripts/aarch64_Linux/bins/grep.yaml" && export BUILD_URL="$BUILD_URL"
       ...
       rclone lsjson --fast-list "r2:/bin/aarch64_arm64_Linux/Baseutils/grep/" --exclude="*.7z" --exclude="*.gz" --exclude="*.jq" --exclude="*.json" --exclude="*.log" --exclude="*.md" --exclude="*.tar" --exclude="*.tgz" --exclude="*.tmp" --exclude="*.txt" --exclude="*.upx" --exclude="*.zip" | \
       jq --arg BUILD_LOG "$BUILD_LOG" --arg BUILD_SCRIPT "$BUILD_SCRIPT" --arg DESCRIPTION "$DESCRIPTION" --arg EXTRA_BINS "$EXTRA_BINS" --arg NOTE "$NOTE" --arg WEB_URL "$WEB_URL" --arg REPO_URL "$REPO_URL" -r 'include "./to_human_bytes" ; .[] | select(.Size != 0 and .Size != -1 and (.Name | test("\\.(7z|bz2|gz|json|md|rar|tar|tgz|tmp|txt|zip)$") | not)) | {name: (.Name), description: $DESCRIPTION, note: $NOTE, download_url: "https://bin.ajam.dev/aarch64_arm64_Linux/Baseutils/grep/\(.Path)", size: (.Size | tonumber | bytes), build_date: (.ModTime | split(".")[0]), repo_url: $REPO_URL, web_url: $WEB_URL, build_log: $BUILD_LOG, build_script: $BUILD_SCRIPT, extra_bins: $EXTRA_BINS}' | jq -s 'sort_by(.name)' > "$TMP_METADIR/INFO.json"
       for BIN in $(cat "$TMP_METADIR/BINS.txt" | sed 's/"//g'); do
         ...
         #BSUM
          B3SUM="$(cat "$BASEUTILSDIR/grep/BLAKE3SUM.txt" | grep --fixed-strings --ignore-case --word-regexp "${BIN}" | awk '{print $1}' | sort  -u | head -n 1 | sed 's/"//g' | sed 's/^[ \t]*//;s/[ \t]*$//' | sed 's/["'\'']//g' | sed 's/`//g' | sed 's/|//g' | tr -d '[:space:]')" && export B3SUM="$B3SUM"
         ...
         #SHA256SUM
          SHA256="$(cat "$BASEUTILSDIR/grep/SHA256SUM.txt" | grep --fixed-strings --ignore-case --word-regexp "${BIN}" | awk '{print $1}' | sort  -u | head -n 1 | sed 's/"//g' | sed 's/^[ \t]*//;s/[ \t]*$//' | sed 's/["'\'']//g' | sed 's/`//g' | sed 's/|//g' | tr -d '[:space:]')" && export SHA256="$SHA256"
       ...
       if jq --exit-status . "$TMP_METADIR/INFO.json" >/dev/null 2>&1; then
          rclone copyto --checksum "$TMP_METADIR/INFO.json" "r2:/bin/aarch64_arm64_Linux/Baseutils/grep/INFO.json" --check-first --checkers 2000 --transfers 1000 --user-agent="$USER_AGENT"
       fi
