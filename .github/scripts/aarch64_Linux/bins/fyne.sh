...
if [ "${SKIP_BUILD}" == "NO" ]; then
    #fyne : Cross platform GUI toolkit in Go inspired by Material Design
     export BIN="fyne"
     export SOURCE_URL="https://github.com/fyne-io/fyne"
     echo -e "\n\n [+] (Building | Fetching) ${BIN} :: ${SOURCE_URL} [$(TZ='UTC' date +'%A, %Y-%m-%d (%I:%M:%S %p)') UTC]\n"
      #Build (alpine-musl)
       ...
       docker run --privileged --net="host" --name "alpine-builder" --pull="always" "azathothas/alpine-builder:latest" \
        ...
        #Build
         git clone --quiet --filter "blob:none" "https://github.com/fyne-io/fyne" && cd "./fyne"
         GOOS="linux" GOARCH="arm64" CGO_ENABLED="1" CGO_CFLAGS="-O2 -flto=auto -fPIE -fpie -static -w -pipe" go build -v -trimpath -buildmode="pie" -ldflags="-s -w -buildid= -linkmode=external -extldflags '\''-s -w -static-pie -Wl,--build-id=none'\''" "./cmd/fyne"
        #strip & info
