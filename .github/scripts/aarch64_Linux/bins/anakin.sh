...
if [ "$SKIP_BUILD" == "NO" ]; then
    #anakin:  Kill orphan processes 
     export BIN="anakin" #Name of final binary/pkg/cli, sometimes differs from $REPO
     export SOURCE_URL="https://github.com/Timmmm/anakin" #github/gitlab/homepage/etc for $BIN
     echo -e "\n\n [+] (Building | Fetching) $BIN :: $SOURCE_URL\n"
      #Build
       pushd "$($TMPDIRS)" > /dev/null 2>&1 && git clone --quiet --filter "blob:none" "https://github.com/Timmmm/anakin" && cd "./anakin"
       ...
       cp "./target/$RUST_TARGET/release/anakin" "$BINDIR/anakin"
       popd > /dev/null 2>&1
fi
#-------------------------------------------------------#
