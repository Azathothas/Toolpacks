...
if [ "${SKIP_BUILD}" == "NO" ]; then
     ...
     export BIN="phantun" #Name of final binary/pkg/cli, sometimes differs from $REPO
     export SOURCE_URL="https://github.com/dndx/phantun" #github/gitlab/homepage/etc for $BIN
     echo -e "\n\n [+] (Building | Fetching) ${BIN} :: ${SOURCE_URL} [$(TZ='UTC' date +'%A, %Y-%m-%d (%I:%M:%S %p)') UTC]\n"
      #Build   
       ...
       cp "./target/$RUST_TARGET/release/client" "$BINDIR/phantun-client"
       cp "./target/$RUST_TARGET/release/server" "$BINDIR/phantun-server" ; popd >/dev/null 2>&1
fi
#-------------------------------------------------------#
