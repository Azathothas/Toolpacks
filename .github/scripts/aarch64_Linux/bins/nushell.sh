...
if [ "$SKIP_BUILD" == "NO" ]; then
  #nushell : A new type of shell 
     ...
     echo -e "\n\n [+] (Building | Fetching) $BIN :: $SOURCE_URL\n"
      #Build
       pushd "$($TMPDIRS)" > /dev/null 2>&1 && git clone --quiet --filter "blob:none" "https://github.com/nushell/nushell" && cd "./nushell"
       ...
       cp "./target/$RUST_TARGET/release/nu" "$BINDIR/nushell"
       cp "./target/$RUST_TARGET/release/nu" "$BINDIR/nu"
       popd > /dev/null 2>&1
fi
#-------------------------------------------------------#
