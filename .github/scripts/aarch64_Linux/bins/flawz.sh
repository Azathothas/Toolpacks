...
if [ "$SKIP_BUILD" == "NO" ]; then
    #flawz: A Terminal UI for browsing security vulnerabilities (CVEs)
     export BIN="flawz" #Name of final binary/pkg/cli, sometimes differs from $REPO
     export SOURCE_URL="https://github.com/orhun/flawz" #github/gitlab/homepage/etc for $BIN
     echo -e "\n\n [+] (Building | Fetching) $BIN :: $SOURCE_URL\n"
      #Build
       ...
       docker run --privileged --net="host" --name "alpine-builder" --pull="always" "azathothas/alpine-builder:latest" \
        ...
        #Build
         git clone --filter "blob:none" --quiet "https://github.com/orhun/flawz" && cd "./flawz"
         echo -e "\n[+] Target: $RUST_TARGET\n"
         echo -e "\n[+] Flags: $RUSTFLAGS\n"
         sed "/^\[profile\.release\]/,/^$/d" -i "./Cargo.toml" ; echo -e "\n[profile.release]\nstrip = true\nopt-level = 3\nlto = true" >> "./Cargo.toml"
         rm rust-toolchain* 2>/dev/null
         cargo build --target "$RUST_TARGET" --release --jobs="$(($(nproc)+1))" --keep-going
         cp "./target/$RUST_TARGET/release/flawz" "/build-bins/flawz"
        '
      #Copy 
       docker cp "alpine-builder:/build-bins/flawz" "./flawz"
       #Meta 
       file "./flawz" && du -sh "./flawz"
       cp "./flawz" "$BINDIR/flawz"
      #Delete Containers
