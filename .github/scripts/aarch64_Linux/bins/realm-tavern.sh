...
if [ "$SKIP_BUILD" == "NO" ]; then
    #realm : parallel | xargs in rust
     export BIN="realm" #Name of final binary/pkg/cli, sometimes differs from $REPO
     ...
     echo -e "\n\n [+] (Building | Fetching) $BIN :: $SOURCE_URL\n"
      ...
      #Server
       GOOS="linux" GOARCH="arm64" CGO_ENABLED="0" go build -v -ldflags="-buildid= -s -w -extldflags '-static'" -o "./tavern-bin" "./tavern" ; cp "./tavern-bin" "$BINDIR/tavern" ; go clean -cache -fuzzcache -modcache -testcache 
      #Agent
       cd "./implants"
