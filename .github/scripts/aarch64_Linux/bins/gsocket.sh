...
if [ "$SKIP_BUILD" == "NO" ]; then
   #gsocket : Connect like there is no firewall. Securely.
     ...
     echo -e "\n\n [+] (Building | Fetching) $BIN :: $SOURCE_URL\n"
      #Fetch
       eval "$EGET_TIMEOUT" eget "$SOURCE_URL" --asset "gsocket" --asset "linux" --asset "aarch64" "$EGET_EXCLUDE" --to "$BINDIR/gsocket"
       eval "$EGET_TIMEOUT" eget "$SOURCE_URL" --asset "gs-netcat" --asset "linux" --asset "aarch64" "$EGET_EXCLUDE" --to "$BINDIR/gs-netcat"
fi
#-------------------------------------------------------#
