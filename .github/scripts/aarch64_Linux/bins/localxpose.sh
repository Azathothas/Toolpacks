...
if [ "$SKIP_BUILD" == "NO" ]; then
    #localxpose : reverse proxy that enables you to expose your localhost to the internet.
     ...
     echo -e "\n\n [+] (Building | Fetching) $BIN :: $SOURCE_URL\n"
      #Fetch
       pushd "$($TMPDIRS)" >/dev/null 2>&1 && curl -A "$USER_AGENT" -qfsSLJO "https://loclx-client.s3.amazonaws.com/loclx-linux-arm64.zip"
       find . -type f -name '*.zip*' -exec unzip -o {} \; && find . -type f -name '*.zip' -exec rm -rf {} \;
       find . -type f -name 'loclx*' -exec strip --strip-debug --strip-dwo --strip-unneeded --preserve-dates "{}" \; >/dev/null 2>&1
