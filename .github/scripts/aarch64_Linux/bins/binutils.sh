    ...
    #binutils: Tools for manipulating binaries (linker, assembler, etc.) (wrapper script)
     ...
     #-------------------------------------------------------#
      ...
      ##Meta
       ...
       #Generate METADATA
       cd "$BASEUTILSDIR/binutils" && find "./" -maxdepth 1 -type f | grep -v -E '\.jq$|\.log$|\.md$|\.png$|\.txt$|\.upx$' | sort | xargs file > "$BASEUTILSDIR/binutils/FILE.txt"
       cd "$BASEUTILSDIR/binutils" && find "./" -maxdepth 1 -type f | grep -v -E '\.jq$|\.log$|\.md$|\.png$|\.txt$|\.upx$' | sort | xargs b3sum > "$BASEUTILSDIR/binutils/BLAKE3SUM.txt"
       cd "$BASEUTILSDIR/binutils" && find "./" -maxdepth 1 -type f | grep -v -E '\.jq$|\.log$|\.md$|\.png$|\.txt$|\.upx$' | sort | xargs sha256sum > "$BASEUTILSDIR/binutils/SHA256SUM.txt"
       ...
       for BIN in $(cat "$TMP_METADIR/BINS.txt" | sed 's/"//g'); do
         ...
         #BSUM
          B3SUM="$(cat "$BASEUTILSDIR/binutils/BLAKE3SUM.txt" | grep -E "^[a-f0-9]+[[:space:]]+(\./)?${BIN}$" | awk '{print $1}' | sort  -u | head -n 1 | sed 's/"//g' | sed 's/^[ \t]*//;s/[ \t]*$//' | sed 's/["'\'']//g' | sed 's/`//g' | sed 's/|//g' | tr -d '[:space:]')" && export B3SUM="$B3SUM"
         ...
         #SHA256SUM
          SHA256="$(cat "$BASEUTILSDIR/binutils/SHA256SUM.txt" | grep -E "^[a-f0-9]+[[:space:]]+(\./)?${BIN}$" | awk '{print $1}' | sort  -u | head -n 1 | sed 's/"//g' | sed 's/^[ \t]*//;s/[ \t]*$//' | sed 's/["'\'']//g' | sed 's/`//g' | sed 's/|//g' | tr -d '[:space:]')" && export SHA256="$SHA256"
