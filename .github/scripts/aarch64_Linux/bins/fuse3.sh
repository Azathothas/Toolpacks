...
if [ "${SKIP_BUILD}" == "NO" ]; then
    #fuse3 : The reference implementation of the Linux FUSE (Filesystem in Userspace) interface
     ...
     #-------------------------------------------------------#
      ...
      #-------------------------------------------------------#
      if [ -s "$HOME/.config/rclone/rclone.conf" ] && [ "$(find "$BASEUTILSDIR/fuse3" -mindepth 1 -print -quit 2>/dev/null)" ]; then
      ##Meta
       ...
       #List
       BUILD_URL="https://pub.ajam.dev/repos/Azathothas/Toolpacks/.github/scripts/aarch64_Linux/bins/fuse3.yaml" && export BUILD_URL="$BUILD_URL"
