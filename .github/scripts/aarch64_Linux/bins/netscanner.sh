...
if [ "$SKIP_BUILD" == "NO" ]; then
     ...
     export BIN="netscanner" #Name of final binary/pkg/cli, sometimes differs from $REPO
     export SOURCE_URL="https://github.com/Chleba/netscanner" #github/gitlab/homepage/etc for $BIN
     echo -e "\n\n [+] (Building | Fetching) $BIN :: $SOURCE_URL\n"
      #Build
       ...
       docker run --privileged --net="host" --name "alpine-builder" "azathothas/alpine-builder:latest" \
        ...
        #Build
         git clone --filter "blob:none" --quiet "https://github.com/Chleba/netscanner" && cd "./netscanner"
         echo -e "\n[+] Target: $RUST_TARGET\n"
         echo -e "\n[+] Flags: $RUSTFLAGS\n"
         sed "/^\[profile\.release\]/,/^$/d" -i "./Cargo.toml" ; echo -e "\n[profile.release]\nstrip = true\nopt-level = 3\nlto = true" >> "./Cargo.toml"
         rm rust-toolchain* 2>/dev/null
         cargo build --target "$RUST_TARGET" --release --jobs="$(($(nproc)+1))" --keep-going
         cp "./target/$RUST_TARGET/release/netscanner" "/build-bins/netscanner"
        '
      #Copy 
       docker cp "alpine-builder:/build-bins/netscanner" "./netscanner"
       #Meta 
       file "./netscanner" && du -sh "./netscanner"
       cp "./netscanner" "$BINDIR/netscanner"
      #Delete Containers
