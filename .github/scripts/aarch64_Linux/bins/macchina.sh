...
if [ "${SKIP_BUILD}" == "NO" ]; then
     ...
     export BIN="macchina" #Name of final binary/pkg/cli, sometimes differs from $REPO
     export SOURCE_URL="https://github.com/Macchina-CLI/macchina" #github/gitlab/homepage/etc for $BIN
     echo -e "\n\n [+] (Building | Fetching) ${BIN} :: ${SOURCE_URL} [$(TZ='UTC' date +'%A, %Y-%m-%d (%I:%M:%S %p)') UTC]\n"
      #Build
       ...
       cargo build --target "$RUST_TARGET" --release --jobs="$(($(nproc)+1))" --keep-going ; cp "./target/$RUST_TARGET/release/macchina" "$BINDIR/macchina" ; popd >/dev/null 2>&1
fi
#-------------------------------------------------------#
