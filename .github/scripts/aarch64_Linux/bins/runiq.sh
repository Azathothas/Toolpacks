...
if [ "$SKIP_BUILD" == "NO" ]; then
     ...
     export BIN="runiq" #Name of final binary/pkg/cli, sometimes differs from $REPO
     export SOURCE_URL="https://github.com/whitfin/runiq" #github/gitlab/homepage/etc for $BIN
     echo -e "\n\n [+] (Building | Fetching) $BIN :: $SOURCE_URL\n"
      #Build
       pushd "$($TMPDIRS)" > /dev/null 2>&1 && git clone --quiet --filter "blob:none" "https://github.com/whitfin/runiq" && cd "./runiq"
       ...
       cargo build --target "$RUST_TARGET" --release --jobs="$(($(nproc)+1))" --keep-going ; cp "./target/$RUST_TARGET/release/runiq" "$BINDIR/runiq" ; popd > /dev/null 2>&1
fi
#-------------------------------------------------------#
