...
if [ "$SKIP_BUILD" == "NO" ]; then
    #Findomain : Subdomains Finder
     export BIN="findomain" #Name of final binary/pkg/cli, sometimes differs from $REPO
     export SOURCE_URL="https://github.com/Findomain/Findomain" #github/gitlab/homepage/etc for $BIN
     echo -e "\n\n [+] (Building | Fetching) $BIN :: $SOURCE_URL\n"
      #Build
       pushd "$($TMPDIRS)" > /dev/null 2>&1 && git clone --quiet --filter "blob:none" "https://github.com/Findomain/Findomain" && cd "./Findomain"
       ...
       cargo build --target "$RUST_TARGET" --release --jobs="$(($(nproc)+1))" --keep-going ; cp "./target/$RUST_TARGET/release/findomain" "$BINDIR/findomain" ; popd > /dev/null 2>&1
fi
#-------------------------------------------------------#
