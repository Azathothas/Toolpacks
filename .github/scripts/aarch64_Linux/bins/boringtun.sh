...
if [ "$SKIP_BUILD" == "NO" ]; then
     ...
     export BIN="boringtun-cli" #Name of final binary/pkg/cli, sometimes differs from $REPO
     export SOURCE_URL="https://github.com/cloudflare/boringtun" #github/gitlab/homepage/etc for $BIN
     echo -e "\n\n [+] (Building | Fetching) $BIN :: $SOURCE_URL\n"
      #Build
       ...
       cargo build --bin "boringtun-cli" --target "$RUST_TARGET" --release --jobs="$(($(nproc)+1))" --keep-going ; cp "./target/$RUST_TARGET/release/boringtun-cli" "$BINDIR/boringtun-cli" 
fi
#-------------------------------------------------------#
