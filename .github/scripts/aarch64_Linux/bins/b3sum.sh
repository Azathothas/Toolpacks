...
if [ "${SKIP_BUILD}" == "NO" ]; then
     ...
     echo -e "\n\n [+] (Building | Fetching) ${BIN} :: ${SOURCE_URL} [$(TZ='UTC' date +'%A, %Y-%m-%d (%I:%M:%S %p)') UTC]\n"
      #Build
       pushd "$($TMPDIRS)" >/dev/null 2>&1 && git clone --quiet --filter "blob:none" "https://github.com/BLAKE3-team/BLAKE3" && cd "./BLAKE3" ; cd "./b3sum"
       ...
       cargo build --target "$RUST_TARGET" --release --jobs="$(($(nproc)+1))" --keep-going ; cp "./target/$RUST_TARGET/release/b3sum" "$BINDIR/b3sum" ; popd >/dev/null 2>&1
fi
#-------------------------------------------------------#
