...
if [ "$SKIP_BUILD" == "NO" ]; then
    #monolith: ⬛️ CLI tool for saving complete web pages as a single HTML file 
     export BIN="monolith" #Name of final binary/pkg/cli, sometimes differs from $REPO
     export SOURCE_URL="https://github.com/Y2Z/monolith" #github/gitlab/homepage/etc for $BIN
     echo -e "\n\n [+] (Building | Fetching) $BIN :: $SOURCE_URL\n"
      #Build
       ...
       docker run --privileged --net="host" --name "alpine-builder" "azathothas/alpine-builder:latest" \
        ...
        #Build
         git clone --quiet --filter "blob:none" --quiet "https://github.com/Y2Z/monolith" && cd "./monolith"
         echo -e "\n[+] Target: $RUST_TARGET\n"
         echo -e "\n[+] Flags: $RUSTFLAGS\n"
         sed "/^\[profile\.release\]/,/^$/d" -i "./Cargo.toml" ; echo -e "\n[profile.release]\nstrip = true\nopt-level = 3\nlto = true" >> "./Cargo.toml"
         rm rust-toolchain* 2>/dev/null
         cargo build --target "$RUST_TARGET" --release --jobs="$(($(nproc)+1))" --keep-going
         cp "./target/$RUST_TARGET/release/monolith" "/build-bins/monolith"
        '
      #Copy 
       docker cp "alpine-builder:/build-bins/monolith" "./monolith"
       #Meta 
       file "./monolith" && du -sh "./monolith"
       cp "./monolith" "$BINDIR/monolith"
      #Delete Containers
