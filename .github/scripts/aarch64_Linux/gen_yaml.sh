...
# bash <(curl -qfsSL "https://pub.ajam.dev/repos/Azathothas/Toolpacks/.github/scripts/aarch64_Linux/gen_yaml.sh")
...
##ENV
 ...
 #Metadata
 curl -qfsSL "https://bin.ajam.dev/aarch64_arm64_Linux/METADATA.json" -o "$SYSTMP/METADATA.json"
 curl -qfsSL "https://bin.ajam.dev/aarch64_arm64_Linux/Baseutils/METADATA.json" -o "$SYSTMP/METADATA_BASEUTILS.json"
#-------------------------------------------------------#
...
for BUILD_URL in $(cat "$SYSTMP/BUILDURLS"); do
   ...
   #Description 
    ...
    echo "Description: ${DESCRIPTION}"
   #WEB_URL
    WEB_URL="$(cat "$BUILDSCRIPT" | grep -o 'export SOURCE_URL="\([^"]*\)"' | sed 's/export SOURCE_URL="\(.*\)"/\1/' | sed 's/ //g' | sed 's/["'\'']//g' | sed 's/`//g' | tr -d '[:space:]')" && export WEB_URL="${WEB_URL}" ; echo "WEB_URL: ${WEB_URL}"
    if [[ "${WEB_URL}" =~ git ]]; then
