...
#rClone Upload to R2 (bin.ajam.dev/aarch64_Linux) (aarch64_Linux) [Binaries]
 if command -v rclone &> /dev/null && [ -s "$HOME/.config/rclone/rclone.conf" ] && [ -d "$BINDIR" ] && [ "$(find "$BINDIR" -mindepth 1 -print -quit 2>/dev/null)" ]; then
    #Upload [$BINDIR]
      ...
      cd "$BINDIR" && rclone copy "." "r2:/bin/aarch64_arm64_Linux/" --user-agent="$USER_AGENT" --s3-upload-concurrency="500" --s3-chunk-size="100M" --multi-thread-streams="500" --checkers="2000" --transfers="1000" --retries="10" --check-first --checksum --copy-links --fast-list --progress && sleep 60
    #Upload [$BASEUTILSDIR]
      echo -e "\n[+] Uploading Results to R2 (Baseutils)\n"
      cd "$BASEUTILSDIR" && rclone copy "." "r2:/bin/aarch64_arm64_Linux/Baseutils/" --user-agent="$USER_AGENT" --s3-upload-concurrency="500" --s3-chunk-size="100M" --multi-thread-streams="500" --checkers="2000" --transfers="1000" --retries="10" --check-first --checksum --copy-links --fast-list --progress && sleep 60
       ...
       if command -v 7z &> /dev/null && [ -d "$BINDIR" ] && [ "$(find "$BINDIR" -mindepth 1 -print -quit 2>/dev/null)" ]; then
          ...
          ##Fetch&Sync [$BINDIR]
            ...
            #Strip || Cleanup
             ...
             find "$BINDIR" -type f -name '*_Linux' -exec sh -c 'newname=$(echo "$1" | sed "s/_aarch64_arm64_Linux//"); mv "$1" "$newname"' sh {} \;
            #File
              cd "$BINDIR" && find "./" -maxdepth 1 -type f | grep -v -E '\.jq$|\.txt$|\.upx$' | sort | xargs file > "$SYSTMP/aarch64_Linux_FILE"
            ...
            #BLAKE3SUM
              cd "$BINDIR" && find "./" -maxdepth 1 -type f | grep -v -E '\.jq$|\.txt$|\.upx$' | sort | xargs b3sum > "$SYSTMP/aarch64_Linux_BLAKE3SUM"
            ...
            #SHA256SUM
              cd "$BINDIR" && find "./" -maxdepth 1 -type f | grep -v -E '\.jq$|\.txt$|\.upx$' | sort | xargs sha256sum > "$SYSTMP/aarch64_Linux_SHA256SUM"
...
if [ "${UPX_PACK}" = "YES" ]; then
   ...
   #Pack:: https://github.com/upx/upx/blob/devel/doc/upx-doc.txt
     if command -v upx &> /dev/null && [ -d "${UPX_PACK_DIR}" ] && [ "$(find "${UPX_PACK_DIR}" -mindepth 1 -print -quit 2>/dev/null)" ]; then
       ...
       #Sync
        rclone copy "r2:/bin/aarch64_arm64_Linux/" "." --user-agent="$USER_AGENT" --buffer-size="10M" --s3-upload-concurrency="50" --s3-chunk-size="10M" --multi-thread-streams="50" --checkers="2000" --transfers="100" --retries="10" --check-first --checksum --copy-links --fast-list --progress && sleep 60
        rclone sync "." "r2:/bin/aarch64_arm64_Linux/" --user-agent="$USER_AGENT" --buffer-size="10M" --s3-upload-concurrency="50" --s3-chunk-size="10M" --multi-thread-streams="50" --checkers="2000" --transfers="100" --retries="10" --check-first --checksum --copy-links --fast-list --progress && sleep 60
     fi
   #Cleanup
     popd >/dev/null 2>&1
