...
set -x && nohup sudo docker run --pull="always" --runtime "sysbox-runc" --network="host" --name="${DOCKER_CONTAINER_NAME}" --rm --env-file="${DOCKER_ENV_FILE}" "${DOCKER_CONTAINER_IMAGE}" > "${DOCKER_LOG_FILE}" 2>&1 &
set +x && sleep 120
#Get logs
