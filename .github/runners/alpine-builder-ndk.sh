...
##Get NDK
  ...
  ndk-pkg install "android-29-arm64-v8a/dos2unix" --profile="release" -j "$(($(nproc)+1))" -v | ansi2txt | tee "./ndk.log"
...
if [[ "${TOOLPACKS_NDK_HOME}" == *"android-ndk"* ]] && [[ "${TOOLPACKS_NDK_ROOT}" == *"android-ndk"* ]] && [[ "${TOOLPACKS_NDK_TOOLCHAIN_ROOT}" == *"android-ndk"* ]]; then
 ...
 ##Symlink
  ln --symbolic --force "${ANDROID_NDK_TOOLCHAIN_BIN}/aarch64-linux-${TOOLPACKS_ANDROID_APILEVEL_DYNAMIC_X}-clang" "/usr/bin/aarch64-linux-android-clang" 2>/dev/null
  ln --symbolic --force "${ANDROID_NDK_TOOLCHAIN_BIN}/clang" "/usr/bin/clang" 2>/dev/null
  #llvm-ar
  ln --symbolic --force "${ANDROID_NDK_TOOLCHAIN_BIN}/llvm-ar" "/usr/bin/aarch64-linux-android-ar" 2>/dev/null
  chmod +x "/usr/bin/aarch64-linux-android-ar" 2>/dev/null
  #llvm-ranlib
  ln --symbolic --force "${ANDROID_NDK_TOOLCHAIN_BIN}/llvm-ranlib" "/usr/bin/aarch64-linux-android-ranlib" 2>/dev/null
